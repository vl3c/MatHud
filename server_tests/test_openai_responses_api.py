...
class TestOpenAIResponsesAPI(unittest.TestCase):
    ...
    def test_convert_messages_to_input_converts_tool_calls_to_text(self, mock_openai: Mock) -> None:
        ...
        api.messages.append({"role": "tool", "tool_call_id": "call_1", "content": "Point created at (5, 0)"})
    ...
    def test_handle_function_call_delta(self, mock_openai: Mock) -> None:
        ...
        event1 = SimpleNamespace(output_index=0, call_id="call_123", name="create_point", delta='{"x":')
        api._handle_function_call_delta(event1, accumulator)

        # Second event with more arguments
        event2 = SimpleNamespace(output_index=0, call_id=None, name=None, delta=" 5}")
    ...
    def test_extract_tool_calls(self, mock_openai: Mock) -> None:
        ...
        response = SimpleNamespace(
            output=[
                SimpleNamespace(
                    type="function_call", call_id="call_456", name="create_circle", arguments='{"radius": 10}'
                )
            ]
        )
    ...
    def test_create_assistant_message(self, mock_openai: Mock) -> None:
        ...
        tool_call = SimpleNamespace(
            id="call_123", function=SimpleNamespace(name="test_func", arguments='{"arg": "value"}')
        )
        response_message = SimpleNamespace(content="Test content", tool_calls=[tool_call])
    ...
    def test_prepare_tool_calls_invalid_json(self, mock_openai: Mock) -> None:
        ...
        api = OpenAIResponsesAPI()
        tool_calls = [{"id": "call_1", "function": {"name": "test", "arguments": "not json"}}]
        ...
        events = [
            ...
            SimpleNamespace(
                ...
                response=SimpleNamespace(
                    ...
                    output=[
                        SimpleNamespace(
                            type="function_call", call_id="call_123", name="create_point", arguments='{"x": 5}'
                        )
                    ],
                ),
            ),
    ...
    def test_create_response_stream_reasoning_placeholder_sent_once(self, mock_openai: Mock) -> None:
        ...
        events = [
            SimpleNamespace(
                type="response.output_item.added", output_index=0, item=SimpleNamespace(type="reasoning", summary=None)
            ),
            SimpleNamespace(
                type="response.output_item.added", output_index=1, item=SimpleNamespace(type="reasoning", summary=None)
            ),
            SimpleNamespace(
                type="response.output_item.added", output_index=2, item=SimpleNamespace(type="reasoning", summary=None)
            ),
        ...
        placeholder_events = [
            e for e in result_events if e.get("type") == "reasoning" and "Reasoning in progress" in e.get("text", "")
        ]
        ...
        events = [
            ...
            SimpleNamespace(
                ...
                response=SimpleNamespace(
                    ...
                    output=[
                        SimpleNamespace(
                            type="function_call", call_id="call_1", name="create_point", arguments='{"x": 1}'
                        ),
                        SimpleNamespace(
                            type="function_call", call_id="call_2", name="create_circle", arguments='{"r": 5}'
                        ),
                    ],
                ),
            ),
        ...
        events = [
            ...
            SimpleNamespace(
                ...
                response=SimpleNamespace(
                    ...
                    output=[
                        SimpleNamespace(
                            type="function_call", call_id="call_123", name="create_point", arguments='{"x": 5, "y": 10}'
                        )
                    ],
                ),
            ),
...
class TestOpenAIResponsesAPIIntegration(unittest.TestCase):
    ...
    def test_integration_reasoning_tokens(self) -> None:
        ...
        total_content = "".join([e.get("text", "") for e in token_events] + [final_events[0].get("ai_message", "")])
