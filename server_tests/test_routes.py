...
class TestRoutes(unittest.TestCase):
    ...
    def test_send_message_uses_canvas_snapshot(self, mock_chat: Mock) -> None:
        ...
        payload = {
            "message": json.dumps({"user_message": "vision request", "use_vision": True}),
            "vision_snapshot": {"renderer_mode": "canvas2d", "canvas_image": canvas_data_url},
        }
    ...
    def test_debug_canvas_state_comparison_is_disabled_when_deployed(self) -> None:
        with (
            patch("static.routes.AppManager.requires_auth", return_value=False),
            patch("static.routes.AppManager.is_deployed", return_value=True),
        ):
...
class TestInterceptSearchTools(unittest.TestCase):
    ...
    def test_handles_alternative_function_key(self, mock_service_class: Mock) -> None:
        ...
        names = [c.get("function_name") or c.get("function", {}).get("name") for c in result]
        self.assertIn("search_tools", names)
        self.assertIn("create_circle", names)
        self.assertNotIn("delete_all", names)
    ...
    def test_injects_tools_into_both_apis(self, mock_service_class: Mock) -> None:
        ...
        # Spy on inject_tools
        with (
            patch.object(self.app.ai_api, "inject_tools") as mock_ai_inject,
