...
class TestOpenAIAPIBase(unittest.TestCase):
    ...
    def test_initialization_custom_model(self, mock_openai: Mock) -> None:
        ...
        custom_model = AIModel.from_identifier("gpt-6-luna")
    ...
    def test_set_model(self, mock_openai: Mock) -> None:
        ...
        api = OpenAIAPIBase()
        api.set_model("google/gemini-3.8-flash")
        self.assertEqual(api.model.id, "google/gemini-3.8-flash")
    ...
    def test_update_tool_messages_with_results(self, mock_openai: Mock) -> None:
        ...
        api = OpenAIAPIBase()
        # Add a tool message
        api.messages.append({"role": "tool", "tool_call_id": "call_123", "content": "Awaiting result..."})
    ...
    @patch("static.openai_api_base.OpenAI")
    @patch("static.openai_api_base._logger")
    def test_prepare_message_content_telemetry_multimodal_reports_output_payload_size(
        self, mock_logger: Mock, mock_openai: Mock
    ) -> None:
        api = OpenAIAPIBase()
        os.environ["AI_CANVAS_SUMMARY_MODE"] = "summary_only"
        os.environ["AI_CANVAS_SUMMARY_TELEMETRY"] = "1"
    ...
    @patch("static.openai_api_base.OpenAI")
    @patch("static.openai_api_base._logger")
    def test_prepare_message_content_telemetry_hybrid_small_scene_infers_full_state(
        self, mock_logger: Mock, mock_openai: Mock
    ) -> None:
        api = OpenAIAPIBase()
        os.environ["AI_CANVAS_SUMMARY_MODE"] = "hybrid"
