...
Follow-ups:
- ~~**Comprehension benchmark for the canvas format**~~ Done: `scripts/benchmark_canvas_formats.py`. On OpenRouter (DeepSeek V4.1 Flash, MiMo V2.6 Pro) both formats answer every static-scene question, and `text` needs about 45% fewer prompt tokens and is faster and cheaper; after a tool batch `json` sends no canvas, so 5 of 8 change questions are not answerable there, while `text` answers all of them. Still open: tune the local budget to the model's context size.
