# MatHud Roadmap Archive (pre-September 2026)

This file keeps the complete roadmap as it stood before the September 2026 rewrite (commit `9be1094`), unchanged, so none of its ideas are lost. The live roadmap is [`documentation/Roadmap.md`](../Roadmap.md): items that fit a personal math-exploration workbench moved there, often rephrased. Everything below is a reference for future work and for possible spinoff apps.

## Spinoff app ideas

Groups of archived sections that could grow into separate apps sharing MatHud's canvas, math engine and AI tool layer.

| Spinoff | What it would be | Archived sections |
|---|---|---|
| **MatHud Mobile** | Android/phone client, touch-first, possibly a thin client to a desktop MatHud server | P0 "Android packaging pipeline"; P4 "Touch and stylus input", "PWA support" |
| **Photo-to-Math Lab** | Take a photo of a real scene and model it mathematically: segmentation, measurement, OCR of handwritten math, curve fitting, physics from images and video | P3 (all: SAM segmentation, mathematical extraction, show-and-tell mode, image-grounded analysis, physics and real-world modeling) |
| **MatHud HUD (AR / wearables)** | Smart-glasses heads-up display that interprets the physical world mathematically | P5 (all: AR canvas overlay, continuous scene understanding, voice-first interaction, live analysis modes, educational, practical and social AR, AR architecture) |
| **MatHud Classroom** | Teaching and learning: shared sessions, teacher/student mode, lessons, quizzes, step-by-step animation | P4 "Collaboration", "Educational", "Step-by-step animation mode"; P5 "Educational AR" |
| **MatHud 3D** | 3D graphing, solids, constraints and camera navigation | P2 "3D (staged after 2D maturity)" |
| **Data & Notebook companion** | Spreadsheet/notebook-style workflows around the canvas: tables, CSV, plots from columns, reusable macros and templates, construction history and replayable sessions | P1 "Data & Tabular Workflows", "Plotting from table columns"; P2 "Construction & Automation", "Workspace & Productivity"; P4 "Notebook-style cell mode", "Shareable workspace links" |

Items from the archive that are still planned for MatHud itself (for example implicit plots, sliders, root/extrema tracing, polar plots, ODEs and direction fields, export, conversation persistence, tabs, a photo underlay) are tracked in the live roadmap and are not repeated here.
