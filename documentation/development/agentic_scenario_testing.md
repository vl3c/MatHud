...
| I5 | Undo accounting, judged by what happened rather than by what the calls reported: a batch that changed the drawables adds exactly one undo entry, a batch that changed none (failed, refused or truthful no-op calls) adds zero, and an undo or redo moves the stack by one. Coordinate-mode changes do not count as drawable changes. | K1 |
