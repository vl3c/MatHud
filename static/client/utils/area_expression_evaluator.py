...
class AreaExpressionEvaluator:
    ...
    def evaluate(expression: str, canvas: "Canvas") -> AreaExpressionResult:
        ...
        try:
            ...
            if isinstance(result, list):
                ...
                return AreaExpressionResult(area=max(0.0, total_area), regions=result)
            elif result is None:
                return AreaExpressionResult(area=0.0, regions=[])
            else:
                area = result.area()
                return AreaExpressionResult(area=max(0.0, area), regions=[result])
    ...
    def _tokenize(expression: str) -> List[str]:
        ...
        while i < len(expr):
            ...
            if char == "(":
                if tokens and not tokens[-1] in AreaExpressionEvaluator.OPERATORS and tokens[-1] != "(":
    ...
    def _evaluate_ast(
        ...
        if isinstance(node, _NameNode):
            ...
            return AreaExpressionEvaluator._drawable_to_region(drawable)
    ...
    def _normalize_to_single(
        result: Union[Region, List[Region], None]
    ) -> Optional[Region]:
    ...
    def _apply_operation(
        left: Union[Region, List[Region], None],
        right: Union[Region, List[Region], None],
        op: str,
    ) -> Union[Region, List[Region], None]:
