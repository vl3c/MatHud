...
class AngleManager:
    ...
    def create_angle(
        ...
        try:
            ...
            segment1 = self.segment_manager.create_segment(
                vertex_point_obj.x,
                vertex_point_obj.y,
                arm1_defining_point_obj.x,
                arm1_defining_point_obj.y,
                extra_graphics=False,
            )
            segment2 = self.segment_manager.create_segment(
                vertex_point_obj.x,
                vertex_point_obj.y,
                arm2_defining_point_obj.x,
                arm2_defining_point_obj.y,
                extra_graphics=False,
            )
    ...
    def get_angle_by_points(
        self, vertex_point: Point, arm1_point: Point, arm2_point: Point, is_reflex_filter: Optional[bool] = None
    ) -> Optional[Angle]:
        """
        ...
        for angle in self.drawables.Angles:
            if not all(
                [
                    hasattr(angle, "vertex_point"),
                    hasattr(angle, "arm1_point"),
                    hasattr(angle, "arm2_point"),
                    hasattr(angle, "is_reflex"),
                ]
            ):
    ...
    def load_angles(self, angles_data: List[Dict[str, Any]]) -> None:
        ...
        for angle_state in angles_data:
            ...
            if new_angle:
                ...
                if (
                    hasattr(new_angle, "segment1")
                    and new_angle.segment1
                    and hasattr(new_angle, "segment2")
                    and new_angle.segment2
                ):
                    ...
                    print(
                        f"AngleManager: Warning - Loaded angle '{new_angle.name}' from state but segments are missing. Cannot register dependencies."
                    )
            else:
    ...
    def get_angles_state(self) -> List[Dict[str, Any]]:
        ...
        return [
            angle.get_state()
            for angle in self.drawables.Angles
            if hasattr(angle, "get_state") and callable(angle.get_state)
        ]
    ...
    def clear_angles(self) -> None:
        ...
        angle_names_to_remove: List[str] = [
            angle.name for angle in cast(List["Drawable"], list(self.drawables.Angles)) if hasattr(angle, "name")
        ]
        ...
        if (
            self.canvas.draw_enabled and not angle_names_to_remove
        ):  # Only draw if nothing was removed (and drawn individually)
            self.canvas.draw()
        elif (
            self.canvas.draw_enabled and angle_names_to_remove
        ):  # If angles were removed, they handled their draw, one final comprehensive draw
            self.canvas.draw()
