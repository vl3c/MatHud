"""
MatHud Octagon Geometric Object

Represents an eight-sided polygon composed of eight connected segments. Validates the
...
class Octagon(Polygon):
    """Represents an eight-sided polygon with regular/irregular metadata."""

    def __init__(self, segments: List[Segment], *, color: str = default_color) -> None:
        if len(segments) != 8:
            raise ValueError("Octagon requires exactly eight segments")
        if not GeometryUtils.segments_form_polygon(segments):
            raise ValueError("Segments do not form a closed octagon")

        ordered_points = GeometryUtils.order_segments_into_loop(segments)
        if ordered_points is None or len(ordered_points) != 8:
            raise ValueError("Unable to determine octagon vertex order")
        ...
        self._set_base_type_labels(["octagon"])
    ...
    def __deepcopy__(self, memo: Dict[int, Any]) -> Octagon:
        if id(self) in memo:
