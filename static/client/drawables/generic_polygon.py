"""
MatHud GenericPolygon Geometric Object

Represents a polygon with 11 or more sides composed of connected segments. Validates
the loop and tags the shape as regular or irregular.
...
class GenericPolygon(Polygon):
    """Represents a polygon with 11+ sides with regular/irregular metadata."""

    MIN_SIDES = 11

    def __init__(self, segments: List[Segment], *, color: str = default_color) -> None:
        if len(segments) < self.MIN_SIDES:
            raise ValueError(f"GenericPolygon requires at least {self.MIN_SIDES} segments")
        if not GeometryUtils.segments_form_polygon(segments):
            raise ValueError("Segments do not form a closed polygon")

        ordered_points = GeometryUtils.order_segments_into_loop(segments)
        if ordered_points is None or len(ordered_points) != len(segments):
            raise ValueError("Unable to determine polygon vertex order")
    ...
    def __deepcopy__(self, memo: Dict[int, Any]) -> GenericPolygon:
        if id(self) in memo:
            return cast(GenericPolygon, memo[id(self)])
