"""
MatHud Nonagon Geometric Object

Represents a nine-sided polygon composed of nine connected segments. Validates the
...
class Nonagon(Polygon):
    """Represents a nine-sided polygon with regular/irregular metadata."""

    def __init__(self, segments: List[Segment], *, color: str = default_color) -> None:
        if len(segments) != 9:
            raise ValueError("Nonagon requires exactly nine segments")
        if not GeometryUtils.segments_form_polygon(segments):
            raise ValueError("Segments do not form a closed nonagon")

        ordered_points = GeometryUtils.order_segments_into_loop(segments)
        if ordered_points is None or len(ordered_points) != 9:
            raise ValueError("Unable to determine nonagon vertex order")
        ...
        self._set_base_type_labels(["nonagon"])
    ...
    def __deepcopy__(self, memo: Dict[int, Any]) -> Nonagon:
        if id(self) in memo:
            return cast(Nonagon, memo[id(self)])
