import math
from config import default_color, default_point_size, default_font_size
...
class Point(Drawable):
    ...
    def draw(self):
        ...
        # Draw label
        label_text = self.name + f'({self.original_position.x}, {self.original_position.y})'
        label_offset = default_point_size
        self.create_svg_element('text', x=str(x+label_offset), y=str(y-label_offset), fill=self.color, text_content=label_text, text_font_size=default_font_size/2)
    ...
    def get_state(self):
        ...
        state = {"class": self.get_class_name(), "args": {"position": {"x": pos.x, "y": pos.y}, "name": self.name}}
        return state