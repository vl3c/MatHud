...
class DrawableNameGenerator:
    ...
    def _get_unique_segment_name_from_string(self, segment_name):
        ...
        segment_name = sorted(self.split_in_2_point_names(segment_name))
    ...
    def get_unique_triangle_name(self, segment_name1, segment_name2, segment_name3):
        ...
        s1_points = self.split_in_2_point_names(segment_name1)
        segment_name2 = self._get_unique_segment_name_from_string(segment_name2)
        s2_points = self.split_in_2_point_names(segment_name2)
        segment_name3 = self._get_unique_segment_name_from_string(segment_name3)
        s3_points = self.split_in_2_point_names(segment_name3)
    ...
    @staticmethod
    def split_in_2_point_names(name):
        ...
        if len(matches) == 2:
            ...
            return sorted([name[0].upper(), name[1].upper()])

    @staticmethod
    def split_in_3_point_names(name):
        # Handle the cases when name is None, an empty string, or a string with only one or two characters
        if name is None or len(name) < 1:
            return ['', '', '']
        elif len(name) == 1:
            return [name.upper(), '', '']
        ...
        if len(matches) == 3:
            ...
            # When there's no match or only one or two matches, return the first three letters of the name.
            return sorted([name[0].upper(), name[1].upper(), name[2].upper()])
    ...
    def delete_segment_name(self, name):
        sorted_name = ''.join(sorted(self.split_in_2_point_names(name)))
        self.segment_names.discard(sorted_name)

    def delete_triangle_name(self, name):
        sorted_name = ''.join(sorted(self.split_in_3_point_names(name)))
        self.segment_names.discard(sorted_name)