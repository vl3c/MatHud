...
class DrawableNameGenerator:
    ...
    def _increment_name(self, function_name):
        # Check if there is a number before the bracket (, if not, append 1; if there is, increment it
        match = re.search(r'(\d+)(?=\()', function_name)
        if match:
            number = int(match.group())
            function_name = function_name[:match.start()] + str(number + 1) + function_name[match.end():]
        else:
            function_name = function_name.split("(")[0] + '1(' + function_name.split("(")[1]
