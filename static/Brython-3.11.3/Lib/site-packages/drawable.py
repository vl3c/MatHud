...
class DrawableNameGenerator:
    ...
    def get_unique_triangle_name_from_segment_names(self, s1_name, s2_name, s3_name):
        print(f"Generating unique triangle name from segments: {s1_name}, {s2_name}, {s3_name}")
        s1_name = self._get_unique_segment_name_from_string(s1_name)
        s1_point_names = self.split_in_n_point_names(s1_name, 2)
        s2_name = self._get_unique_segment_name_from_string(s2_name)
        s2_point_names = self.split_in_n_point_names(s2_name, 2)
        s3_name = self._get_unique_segment_name_from_string(s3_name)
        s3_point_names = self.split_in_n_point_names(s3_name, 2)
        
        # Flatten the list of segments and convert to a set to eliminate duplicates
        unique_points = list(set(s1_point_names + s2_point_names + s3_point_names))
        ...
        if len(unique_points) == 3:
            ...
            all_points = s1_point_names + s2_point_names + s3_point_names
    ...
    def get_unique_triangle_name_from_point_names(self, p1_name, p2_name, p3_name):
        p1_name = self.get_unique_point_name(p1_name)
        p2_name = self.get_unique_point_name(p2_name)
        p3_name = self.get_unique_point_name(p3_name)
        # Extract point names
        point_names = [p1_name, p2_name, p3_name]

        # Convert to a set to eliminate duplicates and then sort
        unique_points = sorted(set(point_names))
        ...
        if len(unique_points) == 3:
            ...
            repeated_point = max(point_names, key=point_names.count)
            name = self._add_unique_triangle_name(unique_points[0], repeated_point, unique_points[1])
        else:
            raise ValueError("Invalid triangle with less than 2 unique points.")
    ...
    def get_unique_rectangle_name_from_segment_names(self, s1_name, s2_name, s3_name, s4_name):
        s1_name = self._get_unique_segment_name_from_string(s1_name)
        s1_point_names = self.split_in_n_point_names(s1_name, 2)
        s2_name = self._get_unique_segment_name_from_string(s2_name)
        s2_point_names = self.split_in_n_point_names(s2_name, 2)
        s3_name = self._get_unique_segment_name_from_string(s3_name)
        s3_point_names = self.split_in_n_point_names(s3_name, 2)
        s4_name = self._get_unique_segment_name_from_string(s4_name)
        s4_point_names = self.split_in_n_point_names(s4_name, 2)
        # Flatten the list of segments and convert to a set to eliminate duplicates
        unique_points = list(set(s1_point_names + s2_point_names + s3_point_names + s4_point_names))
        # Sort the list of unique points
        unique_points.sort()
    ...
    def get_unique_rectangle_name_from_point_names(self, p1_name, p2_name, p3_name, p4_name):
        # Extract and ensure unique point names
        p1_name = self.get_unique_point_name(p1_name)
        p2_name = self.get_unique_point_name(p2_name)
        p3_name = self.get_unique_point_name(p3_name)
        p4_name = self.get_unique_point_name(p4_name)

        # Combine point names into a list
        point_names = [p1_name, p2_name, p3_name, p4_name]

        # Convert to a set to eliminate duplicates and then sort
        unique_points = sorted(set(point_names))

        # Check the number of unique points to determine the rectangle name
        if len(unique_points) == 4:
            name = self._add_unique_rectangle_name(*unique_points)
        else:
            raise ValueError("Invalid rectangle with less than 4 unique points.")
