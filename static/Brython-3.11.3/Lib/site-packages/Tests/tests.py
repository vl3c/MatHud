...
class TestMathFunctions(unittest.TestCase):
    ...
    def test_determine_max_number_of_solutions_cubic_and_quartic_with_higher_order_count(self):
        ...
        result = MathUtil.determine_max_number_of_solutions(equations)
        self.assertEqual(result, 0, "Multiple high-order equations should indicate complex or uncertain scenarios.")
...
class TestProcessFunctionCalls(unittest.TestCase):
    ...
    def test_get_results(self):
        ...
        self.assertEqual(results[0], 25)  # Expected result for "Quadratic(5)"
    ...
    def test_evaluate_expression_invalid_function(self):
        ...
        result = ProcessFunctionCalls.evaluate_expression(expression, variables=None, canvas=self.canvas)
        self.assertTrue("Sorry, that's not a valid mathematical expression." in result)

    def test_validate_results_with_valid_input(self):
        # Testing result validation with valid types
        results = [1, "a", True, 3.14]
        self.assertTrue(ProcessFunctionCalls.validate_results(results))

    def test_validate_results_with_invalid_input(self):
        # Testing result validation with an invalid type (list)
        results = [1, "a", [1, 2, 3], True]
        self.assertFalse(ProcessFunctionCalls.validate_results(results))

    def test_set_function_call_result_with_valid_results(self):
        # Testing setting function call results with valid results
        function_call_results = []
        valid_results = [10, "Test", True]
        ProcessFunctionCalls.set_function_call_result(valid_results, function_call_results)
        self.assertEqual(function_call_results, valid_results)

    def test_set_function_call_result_with_invalid_results(self):
        # Testing setting function call results with invalid results (includes a list)
        function_call_results = []
        invalid_results = [10, "Test", [], True]
        ProcessFunctionCalls.set_function_call_result(invalid_results, function_call_results)
        self.assertEqual(function_call_results, [], "The results list should be empty due to invalid input.")
...
def run_tests():
    ...
    suite.addTest(loader.loadTestsFromTestCase(TestProcessFunctionCalls))
