...
class Function(Drawable):
    ...
    def _generate_values(self):
        ...
        while x <= right_bound:
            point, y = evaluate_point(x, current_path, step)
            
            # Check for discontinuity between previous and current point
            has_discontinuity = False
            if last_point is not None:
                prev_x = x - step
                has_discontinuity = self.has_discontinuity_between_x(prev_x, x)
            
            # Also check for large y-value changes that indicate discontinuity
            if last_y is not None and y is not None and not has_discontinuity:
                y_diff = abs(y - last_y)
                # If y value changes dramatically relative to the visible range
                visible_range = abs(top_bound - bottom_bound)
                if y_diff > visible_range / 4:  # If change is more than 1/4 of visible range
                    has_discontinuity = True
            
            if has_discontinuity:
                # Add current path to paths if it has points
                if current_path:
                    paths.append(current_path)
                    current_path = []
                # Start new path with current point if valid
                if point:
                    current_path = [point]  # Start new path
                    last_point = point
                    last_y = y
            elif point:
    ...
    def has_discontinuity_between_x(self, x1, x2):
        """Check if there is a discontinuity between x1 and x2"""
        return (hasattr(self, 'vertical_asymptotes') and any(x1 <= x <= x2 for x in self.vertical_asymptotes)) or \
               (hasattr(self, 'point_discontinuities') and any(x1 <= x <= x2 for x in self.point_discontinuities)) 