...
class Function(Drawable):
    ...
    def _generate_values(self):
        ...
        while x <= right_bound:
            ...
            # Check for asymptote between previous and current point
            has_asymptote = False
            if hasattr(self, 'vertical_asymptotes') and last_point is not None:
                prev_x = x - step
            ...
            if last_y is not None and y is not None:
                ...
                visible_range = top_bound - bottom_bound
                if y_diff > visible_range / 4:  # If change is more than 1/4 of visible range
                    has_discontinuity = True
            
            if has_asymptote or has_discontinuity:
                # Add current path to points and start a new one
                if current_path:
                    points.extend(current_path)
                    current_path = []
                # Don't reset last_point completely, but start new path with current point if valid
                if point:
