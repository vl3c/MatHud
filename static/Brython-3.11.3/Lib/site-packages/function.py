...
class Function(Drawable):
    ...
    def _crossed_top_bound(self, scaled_y, prev_scaled_y):
        top_bound = 0
        return prev_scaled_y <= top_bound and scaled_y > top_bound

    def _crossed_bottom_bound(self, scaled_y, prev_scaled_y):
    ...
    def _generate_paths(self):
        ...
        last_saved_scaled_point = None

        x = left_bound
        while x <= right_bound:
            if x in self.point_discontinuities or x in self.vertical_asymptotes:
                x += step
                continue
            ...
            crosses_top_bound = False
            crosses_bottom_bound = False

            if last_saved_scaled_point:
                crosses_top_bound = self._crossed_top_bound(scaled_point.y, last_saved_scaled_point.y)
                crosses_bottom_bound = self._crossed_bottom_bound(scaled_point.y, last_saved_scaled_point.y)

            if self._is_out_of_vertical_bounds(scaled_point.y):
                if crosses_top_bound or crosses_bottom_bound:
            ...
            has_vertical_asymptote_behind = False

            prev_x = x - step
            has_vertical_asymptote_behind = self.has_vertical_asymptote_between_x(prev_x, x)

            next_x = x + step
            has_vertical_asymptote_in_front = self.has_vertical_asymptote_between_x(x, next_x)
            ...
            if has_vertical_asymptote_in_front:
                ...
                last_saved_scaled_point = scaled_point
            
            else: # Usual case: add point if visibly different
                pixel_diff = 1  # Minimum pixel difference
                is_first_point = last_saved_scaled_point is None
                if is_first_point or (abs(scaled_point.x - last_saved_scaled_point.x) > pixel_diff) or (abs(scaled_point.y - last_saved_scaled_point.y) > pixel_diff):
                    current_path.append(scaled_point)
                    last_saved_scaled_point = scaled_point
    ...
    def draw(self):
        ...
        # Construct SVG path data with proper path separation
        path_parts = []
        for path in paths:
            if len(path) >= 2:  # Only draw paths with at least 2 points
                path_parts.append("M" + " L".join(f"{p.x},{p.y}" for p in path))
