...
class Function(Drawable):
    ...
    def _generate_paths(self):
        ...
        while x <= right_bound + step:
            ...
            # Check if point is off screen and should be skipped after checking boundary crossings
            ...
            # Only check for asymptote behind if we don't have points in the current path
            if not current_path:
                prev_x = x - step
                has_vertical_asymptote_behind = self.has_vertical_asymptote_between_x(prev_x, x)

            # Only check for asymptote in front if we have points in the current path
            if current_path:
                next_x = x + step
