    ...
    def _generate_paths(self):
        ...
        while x <= right_bound + step:
            ...
            if has_vertical_asymptote_in_front:
                # Determine if function is going up or down before asymptote
                going_down = y < 0 if not current_path else y < self._scaled_to_original(current_path[-1].x, current_path[-1].y).y
                
                # Set first point at top if going down, bottom if going up
                scaled_point.y = self.canvas.cartesian2axis.height if going_down else 0
                current_path.append(scaled_point)
                print("### Added to path: ", scaled_point.x, "(asymptote in front)")

                if len(current_path) == 1:  # Only add second point if we have a single-point path
                    # Add second point at opposite bound
                    second_point = Position(scaled_point.x, 0 if going_down else self.canvas.cartesian2axis.height)
                    current_path.append(second_point)
                    print("### Added SECOND point to path: ", second_point.x, "(asymptote in front)")
                ...
                # It's probably the first point of a new path
                scaled_point.y = self._clamp_y_to_bound(y)
            ...
            if not neighbor_prev_scaled_point:
                x += step
                print("### Skipped: no neighbor previous point")
                continue
            ...
            if is_first_point:
                ...
                print("### Added first point to path -> X: ", scaled_point.x, "Y: ", scaled_point.y)
                x += step
            ...
            if visible_diff:
                current_path.append(scaled_point)
                # print("### Added VISIBLE point to path -> X: ", scaled_point.x, "Y: ", scaled_point.y)
