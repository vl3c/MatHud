...
class Function(Drawable):
    ...
    def _is_out_of_vertical_bounds(self, scaled_y):
        ...
        top_bound = 0
        bottom_bound = self.canvas.cartesian2axis.height
        return scaled_y <= top_bound or scaled_y >= bottom_bound

    def _get_position_on_y_bound(self, scaled_x, unscaled_y):
        """Return a Position object at the vertical bound based on unscaled y value"""
        if unscaled_y > 0:
            return Position(scaled_x, 0)  # 0 is the top bound
        elif unscaled_y < 0:
            return Position(scaled_x, self.canvas.cartesian2axis.height)  # bottom bound
        else:
            return Position(scaled_x, self.canvas.cartesian2axis.origin.y)
    ...
    def _generate_paths(self):
        ...
        while x <= right_bound + step:
            ...
            if x in self.point_discontinuities:
                x += step
                print("### Skipped: point discontinuity")
                continue
            ...
            if not scaled_point:
                x += step
                print("### Skipped: no scaled point")
                continue

            # Check for discontinuity between previous and current point
            has_vertical_asymptote_in_front = False
            has_vertical_asymptote_behind = expect_asymptote_behind  # Use the flag instead of checking

            print("### X: ", x, "Step: ", step, " Asymptotes: ", self.vertical_asymptotes, " \nPrev x: ", x - step, " Next x: ", x + step)

            # Always check for asymptote in front
            has_vertical_asymptote_in_front = self.has_vertical_asymptote_between_x(x, x + step)

            if has_vertical_asymptote_in_front:
                scaled_point = self._get_position_on_y_bound(scaled_point.x, y)
                current_path.append(scaled_point)
                print("### Added to path: ", scaled_point.x, "(asymptote in front)")
                paths.append(current_path)
                current_path = []
                expect_asymptote_behind = True  # Set flag when we detect asymptote in front
                print("### Vertical asymptote in front of point x: ", x, " y: ", y)
                x += step
                print("### Skipped: asymptote in front")
                continue

            elif has_vertical_asymptote_behind:
                # Start new path with current point if valid
                scaled_point = self._get_position_on_y_bound(scaled_point.x, y)
                current_path = [scaled_point]  # Start new path
                print("### Added to path: ", scaled_point.x, "(asymptote behind)")
                expect_asymptote_behind = False  # Clear flag after handling asymptote behind
                print("### Vertical asymptote behind point x: ", x, " y: ", y)
                x += step
                print("### Skipped: asymptote behind")
                continue
            ...
            if not neighbor_prev_scaled_point:
                x += step
                print("### Skipped: no neighbor point")
                continue
            ...
            if x > left_bound:
                ...
                crossed_bound_on_screen = crosses_top_bound_downward or crosses_bottom_bound_upward
                crossed_bound_off_screen = crosses_top_bound_upward or crosses_bottom_bound_downward

                if crossed_bound_on_screen:
                    current_path.append(neighbor_prev_scaled_point) # Add previous off screen point
                    print("### Added to path: ", neighbor_prev_scaled_point.x, "(neighbor point)")
                    current_path.append(scaled_point)
                    print("### Added to path: ", scaled_point.x)
                    x += step
                    print("### Skipped: crossed bound on screen")
                    continue
                ...
                if crossed_bound_off_screen:
                    ...
                    current_path = []
                    x += step
                    print("### Skipped: crossed bound off screen")
                    continue

            if x > right_bound:
                break

            # Check if point is off screen and should be skipped after checking for asymptotes
            if self._is_out_of_vertical_bounds(scaled_point.y):
                x += step
                print("### Skipped: point out of vertical bounds")
                continue

            # Usual case: add point if visibly different
            pixel_diff = 1  # Minimum pixel difference
            is_first_point = not any(current_path)

            if is_first_point:
                current_path.append(scaled_point)
                print("### Added to path: ", scaled_point.x, "(first point)")
                x += step
                print("### Skipped: first point added")
                continue
