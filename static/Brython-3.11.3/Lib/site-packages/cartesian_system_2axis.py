...
class Cartesian2Axis(Drawable):
            ...
            while go():
                if axis == 'x':
                    ...
                    if tick_mark == self.origin.x:
                        # Draw 'O' at origin
                        self.create_svg_element('text', x=str(tick_mark + 2), y=str(self.origin.y + tick_size + self.tick_label_font_size), fill=self.tick_label_color, text_content='O', text_font_size=self.tick_label_font_size)
                    else:
                        # Draw number labels for non-origin ticks
                        tick_label_value = (tick_mark - self.origin.x) / self.canvas.scale_factor
                        tick_label_text = MathUtil.format_number_for_cartesian(tick_label_value)
    ...
    def get_state(self):
        state = {"Cartesian_System_Visibility": {"left_bound": int(self.get_visible_left_bound()), "right_bound": int(self.get_visible_right_bound()), 
                                    "top_bound": int(self.get_visible_top_bound()), "bottom_bound": int(self.get_visible_bottom_bound())}}
        return state