...
class ExpressionValidator(ast.NodeVisitor):
    ...
    ALLOWED_FUNCTIONS = {'sin', 'cos', 'tan', 'sqrt', 'log', 'abs', 'pi'}
    ...
    def evaluate_expression(expression, x=0):
        ...
        variables = {'x': x, 'sin': math.sin, 'cos': math.cos, 'tan': math.tan, 'sqrt': math.sqrt, 'log': math.log, 'abs': abs, 'pi': math.pi}
...
class Utilities:
    ...
    @staticmethod
    def fix_math_expression(string):
        # Replace ^ with **
        string = string.replace('^', '**')
        # Replace |x| with abs(x)
        string = string.replace('|x|', 'abs(x)')
        return string
