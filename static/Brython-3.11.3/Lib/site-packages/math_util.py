...
class ExpressionValidator(ast.NodeVisitor):
    ...
    @staticmethod
    def fix_math_expression(expression, python_power_simbol=False):
    ...
    def parse_function_string(function_string, use_mathjs=False):       
        if use_mathjs:
            ...
            function_string = ExpressionValidator.fix_math_expression(function_string, python_power_simbol=True)
            ExpressionValidator.validate_expression(function_string)
...
class MathUtil:
    @staticmethod
    def format_number(n, max_digits=6):
        """
        ...
        """
        def split_on_e_and_remove_trailing_zeros(formatted_number):
            # Split on 'e', remove trailing zeros from the base, and then reassemble
            base, exponent = formatted_number.split('e')
            base = base.rstrip('0').rstrip('.')
            return f"{base}e{exponent}"
        # If n is an integer or needs scientific notation due to its length
        if n == int(n):
            if abs(n) >= 10**max_digits:
                # Use scientific notation without trailing zeros for integers too large
                return '{:.1e}'.format(n).rstrip('0').rstrip('.')
            else:
                # Integer is within limits, no decimal needed
                return str(int(n))
        # For non-integer numbers that are small enough to potentially be in scientific notation when converted to string
        if abs(n) < 10**(-max_digits):
            # Convert to scientific notation with max_digits precision and remove trailing zeros
            formatted_number = f'{n:.{max_digits}e}'
            if '.' in formatted_number:
                return split_on_e_and_remove_trailing_zeros(formatted_number)
            else:
                return formatted_number
        # If the number is not in scientific notation, format it to have max_digits significant figures
        formatted_number = f"{n:.{max_digits}g}"
        # Check if the formatted number is in scientific notation
        if 'e' in formatted_number:
            return split_on_e_and_remove_trailing_zeros(formatted_number)
        # If it's a normal float (not in scientific notation), just remove the trailing zeros
        return formatted_number.rstrip('0').rstrip('.') if '.' in formatted_number else formatted_number
    ...
    @staticmethod
    def evaluate_expression_using_python_funcs(expression):
        expression = ExpressionValidator.fix_math_expression(expression, python_power_simbol=True)
    ...
    def evaluate(expression, variables=None):
        ExpressionValidator.validate_expression(expression)
        if not variables:
            return float(window.math.format(window.math.evaluate(expression)))
        else:
            # Evaluate the expression using the variables dictionary
            return float(window.math.format(window.math.evaluate(expression, variables)))
    ...
    def limit(expression, variable, value_to_approach):
        # Normalize the value_to_approach
    ...
    def solve_system_of_equations(equations):
        ...
        # Iterating over the jslist and joining each solution into a string
        solution_strings = []
        for solution in solutions:
            solution_strings.append(f"{solution[0]} = {solution[1]}")
    ...
    def test_math_functions():
        ...
        print(f"integral_indefinite('x^2', 'x') = {MathUtil.integral('x^2', 'x')} // Expected: x^3/3")
