...
class DrawableManager:
    ...
    def __init__(self, canvas):
        ...
        # Initialize specialized managers and give them a reference to this manager
        # to solve circular dependency issues
        self.point_manager = PointManager(
            canvas, self.drawables, self.name_generator, self.dependency_manager
        )
        
        self.segment_manager = SegmentManager(
            canvas, self.drawables, self.name_generator, self.dependency_manager, self.point_manager
        )
        ...
        self.triangle_manager = TriangleManager(
            canvas, self.drawables, self.name_generator, self.dependency_manager, 
            self.point_manager, self.segment_manager
        )
        ...
        # Now fix the circular dependency by giving each manager a reference to this DrawableManager
        # This needs to be done after all managers are initialized
        for manager in [self.point_manager, self.segment_manager, self.vector_manager, 
                       self.triangle_manager, self.function_manager, self.circle_manager, 
                       self.ellipse_manager, self.rectangle_manager, self.colored_area_manager]:
            if not hasattr(manager, 'drawable_manager'):
                setattr(manager, 'drawable_manager', self)
