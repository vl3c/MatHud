    ...
    def _split_segments_with_point(self, x, y):
        ...
        for segment in segments.copy():
            ...
            if MathUtils.is_point_on_segment(x, y, sp1.x, sp1.y, sp2.x, sp2.y):
                ...
                if segment1 and segment:
                    self.dependency_manager.register_dependency(child=segment1, parent=segment)
                if segment2 and segment:
                     self.dependency_manager.register_dependency(child=segment2, parent=segment)
                
                # Find geometric children for the new segments and register their dependencies
                if segment1:
                    found_children1 = self.dependency_manager._find_segment_children(segment1)
                    for child in found_children1:
                        self.dependency_manager.register_dependency(child=child, parent=segment1)
                         
                if segment2:
                    found_children2 = self.dependency_manager._find_segment_children(segment2)
                    for child in found_children2:
                        self.dependency_manager.register_dependency(child=child, parent=segment2)