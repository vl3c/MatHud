...
class SegmentManager:
    ...
    def create_segment(self, x1, y1, x2, y2, name="", extra_graphics=True):
        ...
        if extra_graphics:
            ...
            self.drawable_manager._create_drawables_from_new_connections()
    ...
    def _delete_segment_dependencies(self, x1, y1, x2, y2, delete_children=True, delete_parents=False):
        ...
        # Get all parents of this segment for proper relationship management (using reverted method)
        parents = self.dependency_manager._get_segment_parents(segment)
        
        # Recursively delete the segment's children (using reverted method)
        if delete_children:
            self.dependency_manager._delete_segment_children(segment)
            
        # If it's not part of a recursive call, delete the parents (using reverted method)
        if delete_parents:
            self.dependency_manager._delete_segment_parents(segment)
        else:
            # Just remove this segment from its parents' .children attributes
            # Use the reverted _remove_segment_from_parents
            self.dependency_manager._remove_segment_from_parents(segment)
    ...
    def _split_segments_with_point(self, x, y):
        ...
        for segment in segments.copy():
            ...
            if MathUtils.is_point_on_segment(x, y, sp1.x, sp1.y, sp2.x, sp2.y):
                ...
                # Add segments to original parent's .children attribute
                if hasattr(segment, 'children'):
                    segment.children.add(segment1)
                    segment.children.add(segment2)
                
                # Add the new segments to their grandparents' .children using the reverted method
                self.dependency_manager._add_segment_to_parents(segment1) # Uses .children logic now
                self.dependency_manager._add_segment_to_parents(segment2) # Uses .children logic now
                
                # Find children using reverted method and assign to .children attribute
                if hasattr(segment1, 'children'):
                     found_children1 = {c for c in self.dependency_manager._find_segment_children(segment1)} # Uses .children logic
                     segment1.children = found_children1
                     # No registration needed as we are reverting
                          
                if hasattr(segment2, 'children'):
                     found_children2 = {c for c in self.dependency_manager._find_segment_children(segment2)} # Uses .children logic
                     segment2.children = found_children2
                     # No registration needed as we are reverting 