...
class AngleManager:
    ...
    def create_angle(self, vx: float, vy: float, p1x: float, p1y: float, p2x: float, p2y: float, 
                     label: str = None, color: str = None, angle_name: str = None, 
        ...
        Args:
            ...
            angle_name: Optional name for the angle. If not provided, Angle class may generate one.
                               This name is passed as angle_id to the Angle constructor.
        ...
        if not segment1 or not segment2:
            ...
            # TODO: Potential cleanup of points if segments failed and points are orphaned? Or rely on PointManager.
            return None

        # 3. Check if an angle with these exact segments already exists
        existing_angle = self.get_angle_by_segments(segment1, segment2)
        if existing_angle:
            # print(f"AngleManager: Angle with segments {segment1.name} and {segment2.name} already exists: {existing_angle.name}")
        ...
        try:
            new_angle = Angle(
                ...
                label=label, 
                color=color, 
                angle_id=angle_name # Pass angle_name as angle_id for Angle's internal naming
            )
    ...
    def get_angle_by_segments(self, segment1: Segment, segment2: Segment) -> Angle | None:
        """
        ...
        for angle in self.drawables.Angles:
            if not (hasattr(angle, 'segment1') and hasattr(angle, 'segment2')):
                continue # Skip angles that don't have segment attributes properly set up
            if not (angle.segment1 and angle.segment2): 
                continue

            match1 = (angle.segment1 is segment1 and angle.segment2 is segment2)
            match2 = (angle.segment1 is segment2 and angle.segment2 is segment1)
            if match1 or match2:
                return angle
        return None

    def get_angle_by_points(self, vertex_point: Point, arm1_point: Point, arm2_point: Point) -> Angle | None:
        """
        ...
        for angle in self.drawables.Angles:
            if not all([hasattr(angle, 'vertex_point'), hasattr(angle, 'arm1_point'), hasattr(angle, 'arm2_point')]):
                continue # Skip angles missing point attributes
    ...
    def remove_angle(self, angle_name: str) -> bool:
        """
    ...
    def update_angle_properties(self, angle_name: str, new_label: str = None, new_color: str = None) -> bool:
        """
        Updates the properties (label, color) of an existing angle.

        Args:
            angle_name: The name of the angle to update.
            new_label: The new label for the angle. If None, label is not changed.
        ...
        updated = False
        if new_label is not None and hasattr(angle_to_update, 'label'):
            angle_to_update.label = new_label
            updated = True
    ...
    def handle_segment_updated(self, updated_segment_name: str):
            ...
            if angle.segment1.name == updated_segment_name or angle.segment2.name == updated_segment_name:
                if hasattr(angle, '_initialize') and callable(angle._initialize):
                    try:
                        ...
                        self.remove_angle(angle.name) # This will handle its own draw call
        ...
        if needs_redraw and self.canvas.draw_enabled:
            # If remove_angle was called, it might have drawn. A final draw ensures overall consistency.
            self.canvas.draw()
    ...
    def handle_segment_removed(self, removed_segment_name: str):
        ...
        if angles_to_remove_names:
            ...
            # but remove_angle handles its own archiving.
            # self.canvas.undo_redo_manager.archive() 
            ...
            for angle_name in angles_to_remove_names:
                ...
                self.remove_angle(angle_name) # This handles its own draw and archive
            
            # A final draw might not be necessary if remove_angle always draws and is the last action.
            # if self.canvas.draw_enabled:
            #     self.canvas.draw()
    ...
    def load_angles(self, angles_data: list[dict]):
        ...
        for angle_state in angles_data:
            ...
            new_angle = Angle.from_state(angle_state, self.canvas, self.drawable_manager)
    ...
    def clear_angles(self):
        ...
        for angle_name in angle_names_to_remove:
            self.remove_angle(angle_name) # This will handle individual draws and further dependencies.
        
        # A final draw call might be redundant if remove_angle always draws and handles all cleanup.
        # However, if remove_angle was optimized not to draw, this would be needed.
