...
class TestMathFunctions(unittest.TestCase):
    ...
    def test_is_point_on_segment(self):
        ...
        self.assertTrue(
            MathUtil.is_point_on_segment(100.0, 45.332, -122.0, -69.0, 311.0, 154.0),
        ...
        # Point C at y = 100
        self.assertTrue(
            MathUtil.is_point_on_segment(-113.39, 100.0, segment_start_x, segment_start_y, segment_end_x, segment_end_y),
            "Point C (-113.39, 100.0) should be detected as being on segment from (-245.0, 195.0) to (323.0, -215.0)"
        )
        
        # Point D at y = -24
        self.assertTrue(
            MathUtil.is_point_on_segment(58.4, -24.0, segment_start_x, segment_start_y, segment_end_x, segment_end_y),
            "Point D (58.4, -24.0) should be detected as being on segment from (-245.0, 195.0) to (323.0, -215.0)"
        )
        
        # Point E at x = 3
        self.assertTrue(
            MathUtil.is_point_on_segment(3.0, 15.99, segment_start_x, segment_start_y, segment_end_x, segment_end_y),
            "Point E (3.0, 15.99) should be detected as being on segment from (-245.0, 195.0) to (323.0, -215.0)"
        )
        
        # Point F at x = -199
        self.assertTrue(
            MathUtil.is_point_on_segment(-199.0, 161.8, segment_start_x, segment_start_y, segment_end_x, segment_end_y),
        ...
        self.assertTrue(
            MathUtil.is_point_on_segment(point_x, point_y, -122.0, -69.0, 311.0, 154.0),
    ...
    def test_calculate_vertical_asymptotes(self):
        ...
        result = MathUtil.calculate_vertical_asymptotes("1/(x-3)", 0, 6)
    ...
    def test_calculate_asymptotes(self):
        ...
        vert, horiz, disc = MathUtil.calculate_asymptotes_and_discontinuities("1/x", -10, 10)
        ...
        # Test logarithmic function
        vert, horiz, disc = MathUtil.calculate_asymptotes_and_discontinuities("log(x)")
        ...
        vert, horiz, disc = MathUtil.calculate_asymptotes_and_discontinuities("tan(x)", -5, 5)
        # For tan(x), asymptotes occur at x = π/2 + nπ
        ...
        vert, horiz, disc = MathUtil.calculate_asymptotes_and_discontinuities("(x^2+1)/(x^2+2)")
        ...
        vert, horiz, disc = MathUtil.calculate_asymptotes_and_discontinuities("sin(x)")
    ...
    def test_calculate_point_discontinuities(self):
        ...
        result = MathUtil.calculate_point_discontinuities("floor(x)", 0, 2)
        ...
        result = MathUtil.calculate_point_discontinuities("1 if x < 0 else 2 if x <= 1 else 3 if x >= 2 else 4")
