...
class TestDrawableNameGenerator(unittest.TestCase):
    ...
    def test_generate_angle_name_from_segments_valid(self):
        ...
        name = self.generator.generate_angle_name_from_segments("AB", "AC")
        self.assertEqual(name, "angle_BAC")

        # Order of segments shouldn't matter
        name = self.generator.generate_angle_name_from_segments("AC", "AB")
        self.assertEqual(name, "angle_BAC")

        # Different vertex and arm points
        name = self.generator.generate_angle_name_from_segments("XY", "XZ")
        self.assertEqual(name, "angle_YXZ") # Y, Z sorted

        name = self.generator.generate_angle_name_from_segments("CB", "CA")
        self.assertEqual(name, "angle_ACB") # A, B sorted, C is vertex

    def test_generate_angle_name_multi_char_point_names(self):
        # Mocking point_generator.split_point_names to handle multi-char names
        # This is crucial because the actual implementation relies on point_generator
        def mock_split_point_names(segment_name):
            if segment_name == "P1P2": return ["P1", "P2"]
            if segment_name == "P1P3": return ["P1", "P3"]
            if segment_name == "P'Q": return ["P'", "Q"]
            if segment_name == "P'R": return ["P'", "R"]
            if segment_name == "P'P''": return ["P'", "P''"] # Example from point_generator
            if segment_name == "P'P'''": return ["P'", "P'''"]
            return [] # Default empty for unexpected
        
        original_point_splitter = self.generator.point_generator.split_point_names
        self.generator.point_generator.split_point_names = mock_split_point_names
        
        try:
            name = self.generator.generate_angle_name_from_segments("P1P2", "P1P3")
            self.assertEqual(name, "angle_P2P1P3") # P2, P3 sorted around P1

            name = self.generator.generate_angle_name_from_segments("P'Q", "P'R")
            self.assertEqual(name, "angle_QP'R") # Q, R sorted around P'
            
            # Test with apostrophes as in point_generator examples
            name = self.generator.generate_angle_name_from_segments("P'P''", "P'P'''")
            self.assertEqual(name, "angle_P''P'P'''")

        finally:
            # Restore original splitter
            self.generator.point_generator.split_point_names = original_point_splitter
    ...
    def test_generate_angle_name_identical_segments(self):
        name = self.generator.generate_angle_name_from_segments("AB", "AB")
        self.assertIsNone(name) # Will result in 2 unique points, not 3

    def test_generate_angle_name_malformed_segment_names(self):
        # Test cases where split_point_names might return less than 2 points or unexpected results
        def mock_split_malformed(segment_name):
            if segment_name == "A": return ["A"] # Not enough points
            if segment_name == "ABC": return ["A", "B", "C"] # Too many points for a simple segment
            return []
        
        original_splitter = self.generator.point_generator.split_point_names
        self.generator.point_generator.split_point_names = mock_split_malformed
        try:
            name = self.generator.generate_angle_name_from_segments("A", "AB") 
            self.assertIsNone(name)
            name = self.generator.generate_angle_name_from_segments("AB", "C") 
            self.assertIsNone(name)
            name = self.generator.generate_angle_name_from_segments("ABC", "AD")
            self.assertIsNone(name)
        finally:
            self.generator.point_generator.split_point_names = original_splitter
