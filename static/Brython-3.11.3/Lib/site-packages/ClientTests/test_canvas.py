...
class TestCanvas(unittest.TestCase):
    ...
    def test_remove_segment_from_parents(self):
        ...
        self.canvas.drawable_manager.dependency_manager._remove_segment_from_parents(child)
    ...
    def test_get_segment_parents(self):
        ...
        parent1 = self.canvas.drawable_manager.dependency_manager._get_segment_parents(children[0])
        parent2 = self.canvas.drawable_manager.dependency_manager._get_segment_parents(children[1])
    ...
    def test_add_segment_to_parents(self):
        ...
        ab.children.add(cd)
        cd.children.add(ef)
        self.canvas.drawable_manager.dependency_manager._add_segment_to_parents(ef)
        ef_parents = self.canvas.drawable_manager.dependency_manager._get_segment_parents(ef)
    ...
    def test_split_segment_with_specific_coordinates(self):
        ...
        # Calculate a point that's exactly on the segment using linear interpolation
        # We'll use t=0.51 (slightly past halfway) to place the point on the segment
        t = 0.51
        point_c_x = -122.0 * (1-t) + 311.0 * t
        point_c_y = -69.0 * (1-t) + 154.0 * t
        ...
        segment_ab = self.canvas.create_segment(-122.0, -69.0, 311.0, 154.0, name="AB", extra_graphics=False)
        ...
        self.assertEqual(len(segment_ab.children), 2, "Segment AB should have 2 children after splitting")
        
        # Get the child segments (AC and CB)
        ac_segment = self.canvas.get_segment_by_coordinates(-122.0, -69.0, point_c_x, point_c_y)
        cb_segment = self.canvas.get_segment_by_coordinates(point_c_x, point_c_y, 311.0, 154.0)
        ...
        self.assertIn(ac_segment, segment_ab.children, "Segment AC should be a child of AB")
        self.assertIn(cb_segment, segment_ab.children, "Segment CB should be a child of AB")
    ...
    def test_split_segment_comprehensive(self):
        ...
        # Check that the original segment has two children
        self.assertEqual(len(ab.children), 2, "Original segment AB should have 2 children after adding point C")
        ...
        self.assertIn(ac_segment, ab.children, "Segment AC should be a child of AB")
        self.assertIn(cb_segment, ab.children, "Segment CB should be a child of AB")
        ...
        self.assertEqual(len(cb_segment.children), 2, "Segment CB should have 2 children (CD and DB)")
        ...
        cd_parents = self.canvas.drawable_manager.dependency_manager._get_segment_parents(cd_segment)
        db_parents = self.canvas.drawable_manager.dependency_manager._get_segment_parents(db_segment)
    ...
    def test_segment_preservation_with_multiple_points(self):
        """Test that segments are correctly preserved when adding and deleting multiple points on a segment."""
        # Create a parent segment AB
        ab = self.canvas.create_segment(0, 0, 100, 0, name="AB", extra_graphics=False)
        
        # Add 3 points along the segment (at positions 25, 50, and 75)
        ...
        e_point = self.canvas.create_point(75, 0, name="E", extra_graphics=True)
        
        # At this point we should have 10 segments: 
        # AB (parent), AC, CD, DE, EB (child segments from the original)
        # Plus CE, AE, AD, DB, and CB (additional segments created by combinations of points)
        segments = self.canvas.get_drawables_by_class_name('Segment')
        self.assertEqual(len(segments), 10, "Should have 10 segments after adding 3 points")
        
        # Get all the segments
        ac_segment = self.canvas.get_segment_by_coordinates(0, 0, 25, 0)
        cd_segment = self.canvas.get_segment_by_coordinates(25, 0, 50, 0)
        de_segment = self.canvas.get_segment_by_coordinates(50, 0, 75, 0)
        eb_segment = self.canvas.get_segment_by_coordinates(75, 0, 100, 0)
        
        self.assertIsNotNone(ac_segment, "Segment AC should exist")
        self.assertIsNotNone(cd_segment, "Segment CD should exist")
        self.assertIsNotNone(de_segment, "Segment DE should exist")
        self.assertIsNotNone(eb_segment, "Segment EB should exist")
        
        # Verify parent-child relationships
        # When adding multiple points, all possible segments between points become children of the parent
        # Including AC, CD, DE, EB, AD, AE, CE, DB, CB
        self.assertEqual(len(ab.children), 9, "Original segment AB should have 9 children (all possible segments between points)")
        
        # Verify key segments are children
        self.assertIn(ac_segment, ab.children, "Segment AC should be a child of AB")
        self.assertIn(cd_segment, ab.children, "Segment CD should be a child of AB")
        self.assertIn(de_segment, ab.children, "Segment DE should be a child of AB")
        self.assertIn(eb_segment, ab.children, "Segment EB should be a child of AB")
        
        # Now delete the middle point D and verify the segments
        self.canvas.delete_point(50, 0)
        
        # After deleting D, we should have:
        # AB (parent)
        # AC, CE, EB (child segments)
        # AE, CB (segments from combinations of remaining points)
        # We lose CD, DE, AD, DB from the original 10
        segments_after_delete_d = self.canvas.get_drawables_by_class_name('Segment')
        self.assertEqual(len(segments_after_delete_d), 6, "Should have 6 segments after deleting D")
        
        # AC and EB should still exist
        ac_after = self.canvas.get_segment_by_coordinates(0, 0, 25, 0)
        eb_after = self.canvas.get_segment_by_coordinates(75, 0, 100, 0)
        ce_segment = self.canvas.get_segment_by_coordinates(25, 0, 75, 0)
        
        self.assertIsNotNone(ac_after, "Segment AC should still exist after deleting D")
        self.assertIsNotNone(eb_after, "Segment EB should still exist after deleting D")
        self.assertIsNotNone(ce_segment, "New segment CE should exist after deleting D")
        
        # CD and DE should be gone
        cd_after = self.canvas.get_segment_by_coordinates(25, 0, 50, 0)
        de_after = self.canvas.get_segment_by_coordinates(50, 0, 75, 0)
        
        self.assertIsNone(cd_after, "Segment CD should be deleted with point D")
        self.assertIsNone(de_after, "Segment DE should be deleted with point D")
        
        # Now delete point C and verify the segments
        self.canvas.delete_point(25, 0)
        
        # After deleting C, we should have:
        # AB (parent)
        # AE, EB (child segments)
        # The segments AC, CE, and CB are removed
        segments_after_delete_c = self.canvas.get_drawables_by_class_name('Segment')
        self.assertEqual(len(segments_after_delete_c), 3, "Should have 3 segments after deleting C")
        
        # AB and EB should still exist
        ab_after = self.canvas.get_segment_by_name("AB")
        eb_after = self.canvas.get_segment_by_coordinates(75, 0, 100, 0)
        ae_segment = self.canvas.get_segment_by_coordinates(0, 0, 75, 0)
        
        self.assertIsNotNone(ab_after, "Segment AB should still exist after deleting C")
        self.assertIsNotNone(eb_after, "Segment EB should still exist after deleting C")
        self.assertIsNotNone(ae_segment, "New segment AE should exist after deleting C")
        
        # Finally delete point E and verify that only AB remains
        self.canvas.delete_point(75, 0)
        
        # After deleting E, we should lose AE and EB, and have only AB
        segments_after_delete_e = self.canvas.get_drawables_by_class_name('Segment')
        self.assertEqual(len(segments_after_delete_e), 1, "Should have only 1 segment after deleting E")
        
        # AB should still exist
        ab_final = self.canvas.get_segment_by_name("AB")
        self.assertIsNotNone(ab_final, "Original segment AB should still exist after deleting all points")
    ...
    def test_undo_redo_edge_cases(self):
        """Test edge cases with empty undo/redo stacks."""
        # Try to undo/redo with nothing in the stack (should not crash)
        self.canvas.undo()
        self.canvas.redo()
        
        # Create and delete a point
        point_coords = (50, 50)
        point = self.canvas.create_point(point_coords[0], point_coords[1], "PX")
        self.canvas.delete_point(point_coords[0], point_coords[1])
        ...
        # Undo deletion
        self.canvas.undo()
        self.assertIsNotNone(self.canvas.get_point(point_coords[0], point_coords[1]))
