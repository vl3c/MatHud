...
class TestCanvas(unittest.TestCase):
    ...
    def test_segment_preservation_with_multiple_points(self):
        ...
        # --- Reset canvas for Scenario 2: Delete D then C ---
        self.canvas.reset() 

        # Scenario 2: Add C, D. Delete D, then delete C.
        # Re-setup initial conditions for Scenario 2
        ab_s2 = self.canvas.create_segment(0, 0, 100, 0, name="AB", extra_graphics=False)
        c_point_s2 = self.canvas.create_point(25, 0, name="C", extra_graphics=True) # Point C at (25,0)
        d_point_s2 = self.canvas.create_point(50, 0, name="D", extra_graphics=True) # Point D at (50,0)
        # Initial points for S2: A(0,0), B(100,0), C(25,0), D(50,0) - Total 4 points
        # Initial segments for S2: AB, AC, AD, CD, CB, DB - Total 6 segments. Children of AB_s2: AC,AD,CD,CB,DB (5)

        # Verify initial number of points and segments for S2 (optional, but good for sanity)
        self.assertEqual(len(self.canvas.get_drawables_by_class_name('Point')), 4, "S2: Should have 4 points initially")
        self.assertEqual(len(self.canvas.get_drawables_by_class_name('Segment')), 6, "S2: Should have 6 segments initially")

        # Delete D first
        self.canvas.delete_point(50, 0) # Deleting D at (50,0)

        # Checks after D deletion for Scenario 2
        self.assertIsNotNone(self.canvas.get_point(0,0), "S2: Point A should exist after D deletion")
        self.assertIsNotNone(self.canvas.get_point(100,0), "S2: Point B should exist after D deletion")
        self.assertIsNotNone(self.canvas.get_point(25,0), "S2: Point C should exist after D deletion")
        self.assertIsNone(self.canvas.get_point(50,0), "S2: Point D should be gone after D deletion")
        self.assertEqual(len(self.canvas.get_drawables_by_class_name('Point')), 3, "S2: Should be 3 points after D deletion")
        
        all_segments_after_d_del_s2 = self.canvas.get_drawables_by_class_name('Segment')
        self.assertEqual(len(all_segments_after_d_del_s2), 3, "S2: Should have 3 segments after D deletion (AB, AC, CB)")
        
        ac_s2 = self.canvas.get_segment_by_coordinates(0, 0, 25, 0)
        cb_s2 = self.canvas.get_segment_by_coordinates(25, 0, 100, 0)
        self.assertIsNotNone(ac_s2, "S2: AC should exist after D deletion")
        self.assertIsNotNone(cb_s2, "S2: CB should exist after D deletion")
        self.assertIsNone(self.canvas.get_segment_by_coordinates(0, 0, 50, 0), "S2: AD should be gone after D deletion")
        self.assertIsNone(self.canvas.get_segment_by_coordinates(25, 0, 50, 0), "S2: CD should be gone after D deletion")
        self.assertIsNone(self.canvas.get_segment_by_coordinates(50, 0, 100, 0), "S2: DB should be gone after D deletion")

        children_ab_after_d_del_s2 = self.canvas.drawable_manager.dependency_manager.get_children(ab_s2)
        self.assertEqual(len(children_ab_after_d_del_s2), 2, "S2: AB should have 2 children after D deletion (AC, CB)")
        if ac_s2: self.assertIn(ac_s2, children_ab_after_d_del_s2, "S2: AC should be child of AB after D deletion")
        if cb_s2: self.assertIn(cb_s2, children_ab_after_d_del_s2, "S2: CB should be child of AB after D deletion")

        # Now delete point C for Scenario 2
        self.canvas.delete_point(25, 0) # Deleting C at (25,0)

        # Final checks for Scenario 2 (after deleting D then C)
        self.assertIsNotNone(self.canvas.get_point(0,0), "S2: Point A should exist finally")
        self.assertIsNotNone(self.canvas.get_point(100,0), "S2: Point B should exist finally")
        self.assertIsNone(self.canvas.get_point(25,0), "S2: Point C should be gone finally")
        self.assertEqual(len(self.canvas.get_drawables_by_class_name('Point')), 2, "S2: Should be 2 points finally")

        all_segments_final_s2 = self.canvas.get_drawables_by_class_name('Segment')
        self.assertEqual(len(all_segments_final_s2), 1, "S2: Should have 1 segment (AB) after D and C deletion")
        self.assertIsNotNone(self.canvas.get_segment_by_name("AB"), "S2: AB should still exist in the end")
        self.assertIsNone(self.canvas.get_segment_by_coordinates(0, 0, 25, 0), "S2: AC should be gone finally")
        self.assertIsNone(self.canvas.get_segment_by_coordinates(25, 0, 100, 0), "S2: CB should be gone finally")
        
        children_ab_final_s2 = self.canvas.drawable_manager.dependency_manager.get_children(ab_s2)
        self.assertEqual(len(children_ab_final_s2), 0, "S2: AB should have 0 children in the end for scenario 2")
