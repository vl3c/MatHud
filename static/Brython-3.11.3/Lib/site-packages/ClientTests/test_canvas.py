...
class TestCanvas(unittest.TestCase):
    ...
    def test_get_canvas_state_and_reconstruct_preserves_integrity(self):
        ...
        # Points
        # Note: The actual Canvas might auto-name points if not provided, 
        # but for testing reconstruction, explicit naming is better.
        pointA = self.canvas.create_point(10, 20, "A")
        pointB = self.canvas.create_point(30, 40, "B")
        pointC = self.canvas.create_point(50, 50, "C") # For circle center

        # Segment using point objects
        segmentAB = self.canvas.create_segment(pointA.original_position.x, pointA.original_position.y, 
                                              pointB.original_position.x, pointB.original_position.y, "AB")
        
        # Circle centered at pointC
        ...
        # 2. Clear the canvas (drawable_manager.clear_drawables also clears computations)
        self.canvas.clear() 

        # 3. Manually reconstruct the canvas from the original_state
        # This mimics loading a workspace on the client side.

        # Reconstruct Points
        if "Points" in original_state:
            for point_data in original_state["Points"]:
                # Point state: {"name": self.name, "args": {"position": {"x": pos.x, "y": pos.y}}}
        ...
        if "Segments" in original_state:
            for seg_data in original_state["Segments"]:
                # Segment state: {"name": self.name, "args": {"p1": sorted_p1_name, "p2": sorted_p2_name, ...}}
                p1_name = seg_data['args']['p1'] # Key is 'p1', not 'p1_name'
                p2_name = seg_data['args']['p2'] # Key is 'p2', not 'p2_name'
        ...
        if "Circles" in original_state:
            for circ_data in original_state["Circles"]:
                # Circle state: {"name": self.name, "args": {"center": center_point_name, "radius": radius, ...}}
                center_name = circ_data['args']['center'] # Key is 'center', not 'center_name'
                ...
                if center_point:
                     self.canvas.create_circle(center_point.original_position.x, center_point.original_position.y, 
                                              circ_data['args']['radius'], circ_data['name'])
                else:
        ...
        if "Functions" in original_state:
            for func_data in original_state["Functions"]:
                # Function state: {"name": name, "args": {"function_string": str, "left_bound": num, "right_bound": num}}
                args = func_data['args']
                self.canvas.draw_function(args['function_string'], 
                                          func_data['name'],
                                          args.get('left_bound'), # Use .get for optional bounds
                                          args.get('right_bound'))
        
        # Reconstruct Computations
        if "computations" in original_state: # Note: key might be 'Computations' or 'computations'
            for comp_data in original_state["computations"]:
                # comp_data example: {"expression": "area_calc", "result": 500.75}
                self.canvas.add_computation(comp_data["expression"], comp_data["result"])

        # 4. Get the state of the reconstructed canvas
        reconstructed_state = self.canvas.get_canvas_state()

        # 5. Deeply compare the reconstructed state with the original state
        # This comparison might need to be sophisticated if order of elements in lists is not guaranteed.
        # For example, sorting lists of dictionaries by a unique key (e.g., 'name') before comparison.
        # Python's assertEqual for dicts handles nested structures well.
        self.assertEqual(original_state, reconstructed_state,
                         "Reconstructed canvas state does not match the original state.")
