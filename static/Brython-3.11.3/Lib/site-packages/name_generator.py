...
class DrawableNameGenerator:
    ...
    def _increment_function_name(self, func_name):
        ...
        if match:
            ...
            func_name = func_name[:match.start()] + str(number + 1) + func_name[match.end():]
        else:
            # Check if function_name contains '('
            if '(' in func_name:
                parts = func_name.split("(")
                func_name = parts[0] + '1(' + parts[1]
            else:
    ...
    def _generate_unique_function_name(self):
        ...
        for number in count():
            for letter in func_alphabet:
                func_name = f"{letter}{number if number > 0 else ''}(x)"
    ...
    def generate_function_name(self, preferred_name):
        ...
        preferred_name = self.filter_string(preferred_name)
        function_names = self.get_drawable_names('Function')
