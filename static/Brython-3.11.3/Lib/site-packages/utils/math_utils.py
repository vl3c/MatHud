...
class MathUtils:
    # Define a suitable epsilon (tolerance)
    EPSILON = 1e-9
    ...
    def find_diagonal_points(points, rect_name_for_warning):
        ...
        Assumes points are part of an axis-aligned rectangle.
        
        Args:
            points: A list of four Point objects.
            rect_name_for_warning: The name of the rectangle (currently unused if not logging warnings).
            
        Returns:
            A tuple (p_diag1, p_diag2) of diagonal points, or (None, None) if not found.
        """
        if len(points) != 4:
            # window.console.warn(f"MathUtils.find_diagonal_points expected 4 points for rectangle '{rect_name_for_warning}', got {len(points)}. Cannot determine diagonal points.")
            return None, None

        p0 = points[0]
        for i in range(1, 4):
            p_candidate = points[i]
            if p_candidate.original_position.x != p0.original_position.x and \
               p_candidate.original_position.y != p0.original_position.y:
                return p0, p_candidate
        
        p1 = points[1]
        for i in range(2, 4):
            p_candidate = points[i]
            if p_candidate.original_position.x != p1.original_position.x and \
               p_candidate.original_position.y != p1.original_position.y:
                return p1, p_candidate

        if points[0].original_position.x != points[2].original_position.x and \
           points[0].original_position.y != points[2].original_position.y:
            return points[0], points[2]
        
        if points[1].original_position.x != points[3].original_position.x and \
           points[1].original_position.y != points[3].original_position.y:
            return points[1], points[3]

        # window.console.warn(f"MathUtils.find_diagonal_points could not determine a valid pair of diagonal points for rectangle '{rect_name_for_warning}' from its vertices: {[p.name for p in points] if all(hasattr(p, 'name') for p in points) else 'Nameless points'}. Check point coordinates.")
        return None, None
