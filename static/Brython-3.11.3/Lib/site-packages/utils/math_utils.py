...
class MathUtils:
    ...
    def calculate_angle_degrees(vertex_coords, arm1_coords, arm2_coords):
        ...
        # Check for zero-length vectors (points coinciding)
        if (abs(v1x) < MathUtils.EPSILON and abs(v1y) < MathUtils.EPSILON) or \
           (abs(v2x) < MathUtils.EPSILON and abs(v2y) < MathUtils.EPSILON):
            return None # Angle is undefined if an arm has zero length
