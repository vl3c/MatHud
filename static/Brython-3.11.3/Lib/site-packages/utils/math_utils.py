...
class MathUtils:
    ...
    def find_diagonal_points(points, rect_name_for_warning):
        ...
        Finds the two points that are farthest apart (true diagonal).
        ...
        max_distance = 0
        diagonal_pair = (None, None)
        
        for i in range(len(points)):
            for j in range(i + 1, len(points)):
                p1 = points[i]
                p2 = points[j]
                distance = MathUtils.get_2D_distance(p1, p2)
                if distance > max_distance:
                    max_distance = distance
                    diagonal_pair = (p1, p2)
        
        # Verify that we found a valid diagonal pair
        if diagonal_pair[0] is None or diagonal_pair[1] is None:
            return None, None
            
        # Additional validation: ensure the diagonal points are different in both x and y coordinates
        p1, p2 = diagonal_pair
        if (abs(p1.original_position.x - p2.original_position.x) < MathUtils.EPSILON or 
            abs(p1.original_position.y - p2.original_position.y) < MathUtils.EPSILON):
            # The points don't form a valid diagonal (they're on the same horizontal or vertical line)
            return None, None
