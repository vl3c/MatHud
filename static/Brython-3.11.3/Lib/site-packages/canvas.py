...
from geometry import Point, Position, Segment, Vector, Triangle, Rectangle, Circle, Ellipse, Function
...
import copy
from functions_bounded_colored_area import FunctionsBoundedColoredArea
from segments_bounded_colored_area import SegmentsBoundedColoredArea
from function_segment_bounded_colored_area import FunctionSegmentBoundedColoredArea
...
class Canvas:
    ...
    def create_segment(self, x1, y1, x2, y2, name="", extra_graphics=True):
        ...
        # Extract point names from segment name
        point_names = self.name_generator.split_point_names(name, 2)
        # Create new segment with named points
        p1 = self.create_point(x1, y1, point_names[0], extra_graphics=False)
        p2 = self.create_point(x2, y2, point_names[1], extra_graphics=False)
    ...
    def delete_segment(self, x1, y1, x2, y2, delete_children=True, delete_parents=False):
        ...
        if segment:
            ...
            self.delete_colored_areas_for_segment(segment)
            # Then proceed with normal segment deletion
    ...
    def create_vector(self, origin_x, origin_y, tip_x, tip_y, name="", extra_graphics=True):
        ...
        point_names = self.name_generator.split_point_names(name, 2)
        # Create new vector with named points
        origin = self.create_point(origin_x, origin_y, point_names[0], extra_graphics=False)
        tip = self.create_point(tip_x, tip_y, point_names[1], extra_graphics=False)
    ...
    def create_triangle(self, x1, y1, x2, y2, x3, y3, name="", extra_graphics=True):
        ...
        p1 = self.create_point(x1, y1, point_names[0], extra_graphics=False)
        p2 = self.create_point(x2, y2, point_names[1], extra_graphics=False)
        p3 = self.create_point(x3, y3, point_names[2], extra_graphics=False)
    ...
    def delete_function(self, name):
        ...
        if not function:
            return
        # First delete any colored areas using this function
    ...
    def create_colored_area(self, drawable_bound1, drawable_bound2=None, left_bound=None, right_bound=None, color="lightblue", opacity=0.3):
        """
        ...
        Args:
            drawable_bound1: First function/segment (or None for x-axis)
            drawable_bound2: Second function/segment (or None for x-axis)
        ...
        Raises:
            ValueError: If color or opacity values are invalid
        """
        # Validate color and opacity before proceeding
        self._validate_color_and_opacity(color, opacity)
        
        self.archive()  # Archive for undo
        
        if isinstance(drawable_bound1, Segment):
            if drawable_bound2 is not None and not isinstance(drawable_bound2, Segment):
                if isinstance(drawable_bound2, (Function, type(None))) or isinstance(drawable_bound2, (int, float)):
                    # If drawable_bound1 is a segment and drawable_bound2 is a function/None/number, swap them
                    drawable_bound1, drawable_bound2 = drawable_bound2, drawable_bound1
                else:
                    raise ValueError("Invalid combination of arguments")

        if isinstance(drawable_bound1, Segment) and (drawable_bound2 is None or isinstance(drawable_bound2, Segment)):
            # Segment-segment or segment-xaxis case
            colored_area = SegmentsBoundedColoredArea(drawable_bound1, drawable_bound2, canvas=self, color=color, opacity=opacity)
        elif isinstance(drawable_bound2, Segment):
            # Function-segment case (we know drawable_bound1 is not a segment due to the swap above)
            colored_area = FunctionSegmentBoundedColoredArea(drawable_bound1, drawable_bound2, canvas=self, color=color, opacity=opacity)
        else:
            # Function-function case
            colored_area = FunctionsBoundedColoredArea(drawable_bound1, drawable_bound2, canvas=self, 
                                                     left_bound=left_bound, right_bound=right_bound,
                                                     color=color, opacity=opacity)

        self.add_drawable(colored_area)
        self.draw()
        return colored_area

    def delete_colored_area(self, colored_area):
        """Deletes a colored area"""
        if colored_area:
            self.archive()  # Archive for undo
            category = colored_area.get_class_name()
            if category in self.drawables:
                self.drawables[category].remove(colored_area)
                self.draw()

    def delete_colored_area_by_name(self, name):
        ...
        for category in ['FunctionsBoundedColoredArea', 'SegmentsBoundedColoredArea', 'FunctionSegmentBoundedColoredArea']:
            colored_areas = self.drawables.get(category, [])
        ...
        for category in ['FunctionsBoundedColoredArea', 'SegmentsBoundedColoredArea', 'FunctionSegmentBoundedColoredArea']:
            ...
            for area in colored_areas:
                if area.name == name:
                    self.drawables[category].remove(area)
                    self.draw()
                    return
    ...
    def delete_colored_areas_for_function(self, func):
        ...
        colored_areas = self.drawables.get('FunctionsBoundedColoredArea', [])
        for area in colored_areas:
        ...
        colored_areas = self.drawables.get('FunctionSegmentBoundedColoredArea', [])
        for area in colored_areas:
            if area.func == func:
    ...
    def delete_colored_areas_for_segment(self, segment):
        ...
        colored_areas = self.drawables.get('SegmentsBoundedColoredArea', [])
        for area in colored_areas:
    ...
    def _validate_color_and_opacity(self, color, opacity):
        ...
        if not self._is_valid_css_color(color):
    ...
    def update_colored_area_style(self, name, color=None, opacity=None):
        ...
        if color is not None or opacity is not None:
            current_color = color if color is not None else area.color
            current_opacity = opacity if opacity is not None else area.opacity
            self._validate_color_and_opacity(current_color, current_opacity)
            
        # Archive state for undo
        self.archive()
