...
class Canvas:
    ...
    def _delete_point_dependencies(self, x, y):
        ...
        for circle in circles.copy():
            if self._point_matches_coords(circle.center, x, y):
                self.delete_circle(circle.center.x, circle.center.y, circle.radius)
        ellipses = self.drawables.get('Ellipse', [])
        for ellipse in ellipses.copy():
            if self._point_matches_coords(ellipse.center, x, y):
                self.delete_ellipse(ellipse.center.x, ellipse.center.y, ellipse.radius_x, ellipse.radius_y)
    ...
    def create_ellipse(self, center_x, center_y, radius_x, radius_y, name="", extra_graphics=True):
    ...
    def delete_math_function(self, name):
        ...
        for segment in segments.copy():
            ...
            if MathUtil.is_point_on_segment(x, y, sp1.x, sp1.y, sp2.x, sp2.y):
                ...
                if (x, y) != (sp1.x, sp1.y) and (x, y) != (sp2.x, sp2.y):
                    ...
                    new_point = self.create_point(x, y, extra_graphics=False)
                    # Create new segments
