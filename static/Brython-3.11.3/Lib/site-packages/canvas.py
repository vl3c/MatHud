...
class Canvas:
    ...
    def archive(self):
        ...
        #     print(f"Current drawables for archiving: {self.drawables}")
        #     self.drawables_undo_stack.append(copy.deepcopy(self.drawables))
        #     print("Archived drawables")
        #     self.name_generator.archive()


    def undo(self):
        if len(self.drawables_undo_stack) > 0:
            print(f"Current drawables before undo: {self.drawables}")
            print(f"Current undo stack before: {self.drawables_undo_stack}")
            self.drawables_redo_stack.append(copy.deepcopy(self.drawables))
            self.drawables = self.drawables_undo_stack.pop()
            print(f"Current undo stack after: {self.drawables_undo_stack}")
            print(f"Current drawables after undo: {self.drawables}")
            self._fix_drawable_canvas_references()
            self.name_generator.undo()
            self.draw()


    def redo(self):
        if len(self.drawables_redo_stack) > 0:
            print(f"Current drawables before redo: {self.drawables}")
            print(f"Current redo stack before: {self.drawables_redo_stack}")
            self.drawables_undo_stack.append(copy.deepcopy(self.drawables))
            self.drawables = self.drawables_redo_stack.pop()
            print(f"Current redo stack after: {self.drawables_redo_stack}")
            print(f"Current drawables after redo: {self.drawables}")
            self._fix_drawable_canvas_references()
            self.name_generator.redo()
            self.draw()
        ...
        for segment in segments.copy():
            if self._segment_has_point(segment, x, y):
                self.delete_segment(segment.point1.original_position.x, segment.point1.original_position.y, 
                                    ...
                                    delete_children=True, delete_parents=False, archive_state=False)
        vectors = self.drawables.get('Vector', [])
        for vector in vectors.copy():
            if self._segment_has_point(vector.segment, x, y):
                self.delete_vector(vector, archive_state=False)
        circles = self.drawables.get('Circle', [])
        for circle in circles.copy():
            if self._point_matches_coords(circle.center, x, y):
                self.delete_circle(circle.center.x, circle.center.y, circle.radius, archive_state=False)
        ellipses = self.drawables.get('Ellipse', [])
        for ellipse in ellipses.copy():
            if self._point_matches_coords(ellipse.center, x, y):
                self.delete_ellipse(ellipse.center.x, ellipse.center.y, ellipse.radius_x, ellipse.radius_y, archive_state=False)


    def _delete_segment_dependencies(self, x1, y1, x2, y2, delete_children=True, delete_parents=False):
        segment = self.get_segment_by_coord(x1, y1, x2, y2)
        if segment:
        ...
        self.delete_vector(x1, y1, x2, y2, archive_state=False)
        self.delete_vector(x2, y2, x1, y1, archive_state=False)
    ...
    def create_point(self, x, y, name="", create_extra_graphics=True):
        point = self.get_point(x, y)
        if not point:
            # Create new point
            self.archive()
            name = self.name_generator.get_unique_point_name(name)
            point = Point(x, y, canvas=self, name=name)
            self.add_drawable(point)
            if create_extra_graphics:
                self._create_drawables_from_new_connections()
            self.draw()
        return point


    def delete_point(self, x, y, archive_state=True):
        point = self.get_point(x, y)
        if point:
            if archive_state:
                self.archive()
    ...
    def get_segment_by_coord(self, x1, y1, x2, y2):
        segments = self.drawables.get('Segment', [])
        for segment in segments.copy():
    ...
    def get_segment_by_points(self, p1, p2):
        return self.get_segment_by_coord(p1.x, p1.y, p2.x, p2.y)


    def create_segment(self, x1, y1, x2, y2, name="", create_extra_graphics=True):
        existing_segment = self.get_segment_by_coord(x1, y1, x2, y2)
        if existing_segment:
            return existing_segment
        self.archive()
        # point_names = self.name_generator.split_in_n_point_names(name, 2)
        p1 = self.create_point(x1, y1, create_extra_graphics=False)
        p2 = self.create_point(x2, y2, create_extra_graphics=False)
    ...
    def delete_segment(self, x1, y1, x2, y2, delete_children=True, delete_parents=True, archive_state=True):
        segment = self.get_segment_by_coord(x1, y1, x2, y2)
        if segment:
            if archive_state:
                self.archive()
    ...
    def any_segment_part_visible_in_canvas_area(self, x1, y1, x2, y2):
        # print(f"Segment endpoints: ({x1}, {y1}), ({x2}, {y2})")      # DEBUG
        # print(f"Canvas dimensions: {self.width} x {self.height}")    # DEBUG
        intersect_top = Utilities.segments_intersect(x1, y1, x2, y2, 0, 0, self.width, 0)
        intersect_right = Utilities.segments_intersect(x1, y1, x2, y2, self.width, 0, self.width, self.height)
        intersect_bottom = Utilities.segments_intersect(x1, y1, x2, y2, self.width, self.height, 0, self.height)
        intersect_left = Utilities.segments_intersect(x1, y1, x2, y2, 0, self.height, 0, 0)
        # print(f"Intersects with top: {intersect_top}")        # DEBUG
        # print(f"Intersects with right: {intersect_right}")    # DEBUG
        # print(f"Intersects with bottom: {intersect_bottom}")  # DEBUG
        # print(f"Intersects with left: {intersect_left}")      # DEBUG
    ...
    def create_vector(self, origin_x, origin_y, tip_x, tip_y, name=""):
        ...
        self.archive()
        #point_names = self.name_generator.split_in_n_point_names(name, 2)
        origin = self.create_point(origin_x, origin_y, create_extra_graphics=False)
        tip = self.create_point(tip_x, tip_y, create_extra_graphics=False)
        vector = Vector(origin, tip, canvas=self, name=name)
        self.add_drawable(vector)
        self.draw()
        return vector


    def delete_vector(self, origin_x, origin_y, tip_x, tip_y, archive_state=True):
    ...
    def create_triangle(self, x1, y1, x2, y2, x3, y3, name="", create_extra_graphics=True):
        existing_triangle = self.get_triangle(x1, y1, x2, y2, x3, y3)
        if existing_triangle:
            return existing_triangle
        self.archive()
        s1 = self.create_segment(x1, y1, x2, y2, create_extra_graphics=False)
        s2 = self.create_segment(x2, y2, x3, y3, create_extra_graphics=False)
        s3 = self.create_segment(x3, y3, x1, y1, create_extra_graphics=False)
        print(f"Created segments: {s1.name}, {s2.name}, {s3.name}")    # DEBUG
        triangle_name = self.name_generator.get_unique_triangle_name_from_segment_names(s1.name, s2.name, s3.name)
        triangle = Triangle(s1, s2, s3, canvas=self, name=triangle_name)
    ...
    def delete_triangle(self, x1, y1, x2, y2, x3, y3, archive_state=True):     
    ...
    def create_rectangle(self, diag_p1_x, diag_p1_y, diag_p2_x, diag_p2_y):
        # Check if the rectangle already exists
        existing_rectangle = self.get_rectangle_by_diagonal_points(diag_p1_x, diag_p1_y, diag_p2_x, diag_p2_y)
        if existing_rectangle:
            return existing_rectangle
        self.archive()
        # Create or retrieve the segments of the rectangle
        s1 = self.create_segment(diag_p1_x, diag_p1_y, diag_p2_x, diag_p1_y, create_extra_graphics=False)
        s2 = self.create_segment(diag_p2_x, diag_p1_y, diag_p2_x, diag_p2_y, create_extra_graphics=False)
        s3 = self.create_segment(diag_p2_x, diag_p2_y, diag_p1_x, diag_p2_y, create_extra_graphics=False)
        s4 = self.create_segment(diag_p1_x, diag_p2_y, diag_p1_x, diag_p1_y, create_extra_graphics=False)
    ...
    def delete_rectangle(self, name, archive_state=True):
    ...
    def get_circle(self, center_x, center_y, radius):
        ...
        for circle in circles:
            if circle.center.original_position.x == center_x and circle.center.original_position.y == center_y and circle.original_radius == radius:
                return circle
        return None
    ...
    def create_circle(self, center_x, center_y, radius, name=""):
        existing_circle = self.get_circle(center_x, center_y, radius)
        if existing_circle:
            return existing_circle
        self.archive()
        center = self.create_point(center_x, center_y, create_extra_graphics=False)
        circle_name = self.name_generator.get_unique_circle_name(name)
        circle = Circle(center, radius, canvas=self, name=circle_name)
        self.add_drawable(circle)
        self.draw()
        return circle


    def delete_circle(self, name, archive_state=True):
        ...
        self.name_generator.delete_circle_name(circle.name)
        self.draw()
    ...
    def get_ellipse(self, center_x, center_y, radius_x, radius_y):
        ...
        for ellipse in ellipses:
            if ellipse.center.original_position.x == center_x and ellipse.center.original_position.y == center_y and ellipse.original_rx == radius_x and ellipse.original_ry == radius_y:
                return ellipse
        return None
    ...
    def create_ellipse(self, center_x, center_y, radius_x, radius_y, name=""):
        existing_ellipse = self.get_ellipse(center_x, center_y, radius_x, radius_y)
        if existing_ellipse:
            return existing_ellipse
        self.archive()
        center = self.create_point(center_x, center_y, create_extra_graphics=False)
        elipse_name = self.name_generator.get_unique_ellipse_name(name)
        ellipse = Ellipse(center, radius_x, radius_y, canvas=self, name=elipse_name)
        self.add_drawable(ellipse)
        self.draw()
        return ellipse


    def delete_ellipse(self, name, archive_state=True):
        ...
        self.name_generator.delete_ellipse_name(ellipse.name)
        self.draw()
    ...
    def draw_math_function(self, function_string, name, left_bound=None, right_bound=None):
        ...
        if existing_function:
            ...
            existing_function.function_string = Utilities.fix_math_expression(function_string)
            try:
                existing_function.function = Utilities.parse_function_string(function_string)
    ...
    def delete_math_function(self, name, archive_state=True):
        ...
        self.name_generator.delete_function_name(function.name)
        self.draw()


    @staticmethod
    def _get_unique_point_names(segments):
        ...
        for i in range(len(segments)):
            for j in range(i+1, len(segments)):
                for k in range(j+1, len(segments)):
                    ...
                    unique_points = self._get_unique_point_names([s1, s2, s3])
                    ...
                    if len(unique_points) == 3 and self._are_points_connected(unique_points):
                        ...
                        self.create_triangle(p1.x, p1.y, p2.x, p2.y, p3.x, p3.y)


    def _split_segments_if_needed(self, x, y):
        segments = self.drawables.get('Segment', [])
        for segment in segments.copy():
            ...
            if Utilities.is_point_on_segment(x, y, sp1.x, sp1.y, sp2.x, sp2.y):
                ...
                if (x, y) != (sp1.x, sp1.y) and (x, y) != (sp2.x, sp2.y):
                    ...
                    new_point = self.create_point(x, y)
                    # Create new segments
                    segment1 = self.create_segment(new_point.original_position.x, new_point.original_position.y, sp1.x, sp1.y, create_extra_graphics=False) 
                    segment2 = self.create_segment(new_point.original_position.x, new_point.original_position.y, sp2.x, sp2.y, create_extra_graphics=False)
    ...
    def _create_drawables_from_new_connections(self):
        points = self.drawables.get('Point', [])
        for point in points:
            self._split_segments_if_needed(point.original_position.x, point.original_position.y)