...
class Canvas:
    ...
    def undo(self):
        ...
        if not self.drawables_undo_stack:
            return
            
        # Get the last archived state
        last_state = self.drawables_undo_stack.pop()
        
        # Archive current state for redo
        current_state = copy.deepcopy(self.drawables)
        self.drawables_redo_stack.append(current_state)
        
        # Restore the last state
        self.drawables = copy.deepcopy(last_state)  # Make sure we deep copy the restored state
    ...
    def redo(self):
        ...
        current_state = copy.deepcopy(self.drawables)
        self.drawables_undo_stack.append(current_state)
        
        # Restore the next state
        self.drawables = copy.deepcopy(next_state)  # Deep copy the restored state
    ...
    def translate_object(self, name, x_offset, y_offset):
        ...
        # Apply translation based on object type
        if isinstance(drawable, Function):
            # Functions don't translate
            pass
        elif hasattr(drawable, 'original_position'):
            # For points and objects with a single position
            drawable.original_position.x += x_offset
            drawable.original_position.y += y_offset
        elif hasattr(drawable, 'center'):
            # For circles and ellipses
            drawable.center.original_position.x += x_offset
            drawable.center.original_position.y += y_offset
        elif isinstance(drawable, Vector):
            # For vectors (translate both the segment points)
            drawable.segment.point1.original_position.x += x_offset
            drawable.segment.point1.original_position.y += y_offset
            drawable.segment.point2.original_position.x += x_offset
            drawable.segment.point2.original_position.y += y_offset
        elif isinstance(drawable, Triangle):
            # For triangles (translate all three segments' points)
            drawable.segment1.point1.original_position.x += x_offset
            drawable.segment1.point1.original_position.y += y_offset
            drawable.segment1.point2.original_position.x += x_offset
            drawable.segment1.point2.original_position.y += y_offset
            drawable.segment2.point2.original_position.x += x_offset
            drawable.segment2.point2.original_position.y += y_offset
        elif isinstance(drawable, Rectangle):
            # For rectangles (translate all four segments' points)
            drawable.segment1.point1.original_position.x += x_offset
            drawable.segment1.point1.original_position.y += y_offset
            drawable.segment1.point2.original_position.x += x_offset
            drawable.segment1.point2.original_position.y += y_offset
            drawable.segment2.point2.original_position.x += x_offset
            drawable.segment2.point2.original_position.y += y_offset
            drawable.segment3.point2.original_position.x += x_offset
            drawable.segment3.point2.original_position.y += y_offset
        elif hasattr(drawable, 'point1') and hasattr(drawable, 'point2'):
            # For segments
            drawable.point1.original_position.x += x_offset
            drawable.point1.original_position.y += y_offset
            drawable.point2.original_position.x += x_offset
            drawable.point2.original_position.y += y_offset
        
        # Update the object's position in the canvas coordinate system
        if hasattr(drawable, '_initialize'):
            drawable._initialize()
