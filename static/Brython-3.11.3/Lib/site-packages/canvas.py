...
class Canvas:
    ...
    def _delete_point_dependencies(self, x, y):
        ...
        for segment in segments_to_delete:
            ...
            for parent in parents:
                if not MathUtil.segment_has_end_point(parent, x, y):
    ...
    def _delete_segment_dependencies(self, x1, y1, x2, y2, delete_children=True, delete_parents=False):
        ...
        rectangles = self.drawables.get('Rectangle', [])
        for rectangle in rectangles.copy():
            if any(MathUtil.segment_matches_coordinates(s, x1, y1, x2, y2) for s in [rectangle.segment1, rectangle.segment2, rectangle.segment3, rectangle.segment4]):
                rectangles.remove(rectangle)
            
        # Delete the triangles that contain the segment
        triangles = self.drawables.get('Triangle', [])
        for triangle in triangles.copy():
            if any(MathUtil.segment_matches_coordinates(s, x1, y1, x2, y2) for s in [triangle.segment1, triangle.segment2, triangle.segment3]):
                triangles.remove(triangle)
    ...
    def create_colored_area(self, drawable1_name, drawable2_name=None, left_bound=None, right_bound=None, color="lightblue", opacity=0.3):
        ...
        if isinstance(drawable1, Segment) and (drawable2 is None or isinstance(drawable2, Segment)):
            ...
            colored_area = SegmentsBoundedColoredArea(drawable1, drawable2, canvas=self, color=color, opacity=opacity)
        elif isinstance(drawable2, Segment):
            # Function-segment case (we know drawable1 is not a segment due to the swap above)
            colored_area = FunctionSegmentBoundedColoredArea(drawable1, drawable2, canvas=self, color=color, opacity=opacity)
        else:
            # Function-function case
            colored_area = FunctionsBoundedColoredArea(drawable1, drawable2, canvas=self, 
