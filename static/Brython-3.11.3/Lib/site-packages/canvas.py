...
class Canvas:
    ...
    def get_segment_by_coordinates(self, x1, y1, x2, y2):
        segments = self.drawables.get('Segment', [])
        for segment in segments.copy():
            if MathUtils.segment_has_end_point(segment, x1, y1) and MathUtils.segment_has_end_point(segment, x2, y2):
                return segment
        return None
    ...
    def get_vector(self, x1, y1, x2, y2):
        vectors = self.drawables.get('Vector', [])
        for vector in vectors.copy():
            if MathUtils.segment_has_end_point(vector.segment, x1, y1) and MathUtils.segment_has_end_point(vector.segment, x2, y2):
                return vector
        return None
    ...
    def get_triangle(self, x1, y1, x2, y2, x3, y3):
        ...
        for drawable in triangles.copy():
            if all(MathUtils.segment_matches_coordinates(segment, x1, y1, x2, y2) or
                ...
                MathUtils.segment_matches_coordinates(segment, x2, y2, x3, y3) for segment in [drawable.segment1, drawable.segment2, drawable.segment3]):
                return drawable
        return None
    ...
    def _is_fully_connected_graph(self, list_of_point_names):
        return GeometryUtils.is_fully_connected_graph(list_of_point_names, self.drawables.get('Segment', []))
