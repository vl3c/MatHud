    ...
    def _delete_point_dependencies(self, x, y):
        ...
        for segment in segments.copy():
            if self._segment_has_point(segment, x, y):
                self.delete_segment(segment.point1.original_position.x, segment.point1.original_position.y, segment.point2.original_position.x, segment.point2.original_position.y)
        vectors = self.drawables.get('Vector', [])
        for vector in vectors.copy():
            if self._segment_has_point(vector.segment, x, y):
                vectors.remove(vector)

    def _delete_segment_dependencies(self, x1, y1, x2, y2, delete_children, delete_parents):
