...
class Vector(Drawable):
    ...
    def __deepcopy__(self, memo):
        ...
        new_vector = Vector(new_origin, new_tip, self.canvas, name=self.name, color=self.color)
...
class Function(Drawable):
    def __init__(self, function_string, canvas, name="", step=default_point_size, color=default_color, left_bound=None, right_bound=None):
        ...
        self.step = step
        self.function_string = Utilities.fix_math_expression(function_string)
    ...
    def draw(self):
        ...
        # Draw function name
        label_offset = point_label_font_size
        name_x = xy[1].x
        name_y = xy[1].y + label_offset
        self.create_svg_element('text', x=str(name_x), y=str(name_y), fill=self.color, text_content=self.name, text_font_size=point_label_font_size)
        # Draw function
        for i in range(len(xy) - 1):
            p1 = xy[i]
            p2 = xy[i + 1]
            self.create_svg_element('line', x1=str(p1.x), y1=str(p1.y), x2=str(p2.x), y2=str(p2.y), stroke=self.color)
