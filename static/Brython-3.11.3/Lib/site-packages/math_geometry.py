...
class Segment(Drawable):
    def __init__(self, x1, y1, x2, y2, canvas, name="", color=default_color):
        self._create_points(x1, y1, x2, y2, canvas, name, color)
        name = canvas.name_generator.get_unique_segment_name(self.point1.name, self.point2.name)
        super().__init__(name=name, color=color, canvas=canvas)
        self._initialize()
        self.children = []
    
    def _create_points(self, x1, y1, x2, y2, canvas, name, color):
        p_names = canvas.name_generator.split_in_n_point_names(name, 2)
        p1 = canvas.get_point(x1, y1)
        if p1:
            self.point1 = p1
        else:
            self.point1 = Point(x1, y1, canvas=canvas, name=p_names[0], color=color)
            canvas.add_drawable(self.point1)
        p2 = canvas.get_point(x2, y2)
        if p2:
            self.point2 = p2
        else:
            self.point2 = Point(x2, y2, canvas=canvas, name=p_names[1], color=color)
    ...
    def get_state(self):
        ...
        state = {"name": self.name, "args": {"p1": points_names[0], "p2": points_names[1]}}
        return state


class Triangle(Drawable):
    def __init__(self, x1, y1, x2, y2, x3, y3, canvas, name="", color=default_color):
        self._create_segments(x1, y1, x2, y2, x3, y3, canvas, name, color)
        name = canvas.name_generator.get_unique_triangle_name(self.segment1.name, self.segment2.name, self.segment3.name)
        super().__init__(name=name, color=color, canvas=canvas)
        self._initialize()

    def _create_segments(self, x1, y1, x2, y2, x3, y3, canvas, name, color):
        point_names = canvas.name_generator.split_in_n_point_names(name, 3)
        s1 = canvas.get_segment(x1, y1, x2, y2)
        if s1:
            self.segment1 = s1
        else:
            segment_name = point_names[0] + point_names[1]
            self.segment1 = Segment(x1, y1, x2, y2, name=segment_name, color=color, canvas=canvas)
            canvas.add_drawable(self.segment1)
        s2 = canvas.get_segment(x2, y2, x3, y3)
        if s2:
            self.segment2 = s2
        else:
            segment_name = point_names[1] + point_names[2]
            self.segment2 = Segment(x2, y2, x3, y3, name=segment_name, color=color, canvas=canvas)
            canvas.add_drawable(self.segment2)
        s3 = canvas.get_segment(x3, y3, x1, y1)
        if s3:
            self.segment3 = s3
        else:
            segment_name = point_names[2] + point_names[1]
            self.segment3 = Segment(x3, y3, x1, y1, name=segment_name, color=color, canvas=canvas)
...
class Rectangle(Drawable):
    def __init__(self, px, py, opposite_px, opposite_py, canvas, name="", color=default_color):
        self._create_segments(px, py, 
                              opposite_px, py, 
                              opposite_px, opposite_py, 
                              px, opposite_py, 
                              canvas, name, color)
        name = canvas.name_generator.get_unique_rectangle_name(self.segment1.name, self.segment2.name, self.segment3.name, self.segment4.name)
        super().__init__(name=name, color=color, canvas=canvas)
        self._initialize()

    def _create_segments(self, x1, y1, x2, y2, x3, y3, x4, y4, canvas, name, color):
        point_names = canvas.name_generator.split_in_n_point_names(name, 4)
        s1 = canvas.get_segment(x1, y1, x2, y2)
        if s1:
            self.segment1 = s1
        else:
        ...
        if s2:
            ...
            self.segment2 = Segment(x2, y2, x3, y3, name=segment_name, color=color, canvas=canvas)
            canvas.add_drawable(self.segment2)
        s3 = canvas.get_segment(x3, y3, x4, y4)
        if s3:
            self.segment3 = s3
        else:
            segment_name = point_names[2] + point_names[3]
            self.segment3 = Segment(x3, y3, x4, y4, name=segment_name, color=color, canvas=canvas)
            canvas.add_drawable(self.segment3)
        s4 = canvas.get_segment(x4, y4, x1, y1)
        if s4:
            self.segment4 = s4
        else:
            segment_name = point_names[3] + point_names[0]
            self.segment4 = Segment(x4, y4, x1, y1, name=segment_name, color=color, canvas=canvas)
            canvas.add_drawable(self.segment4)
...
class Circle(Drawable):
    def __init__(self, center_x, center_y, radius, canvas, name="", color=default_color):
        self.original_radius = radius
        self.center = canvas.get_point(center_x, center_y)
        if not self.center:
            self.center = Point(center_x, center_y, canvas=canvas, name=name, color=color)
            canvas.add_drawable(self.center)
        name = canvas.name_generator.get_unique_circle_name(self.center.name)
        super().__init__(name=name, color=color, canvas=canvas)
        self._initialize()
    ...
    def zoom(self):
        self.radius = self.original_radius * self.canvas.scale_factor
        # Panning is done by the canvas for the center point  
        # self.center.zoom()
    ...
    def get_state(self):
        ...
        state = {"name": self.name, "args": {"center": center, "radius": radius}}
        return state


class Ellipse(Drawable):
    def __init__(self, center_x, center_y, rx, ry, canvas, name="", color=default_color):
        self.original_rx = rx
        self.original_ry = ry
        self.center = canvas.get_point(center_x, center_y)
        if not self.center:
            self.center = Point(center_x, center_y, canvas=canvas, name=name, color=color)
            canvas.add_drawable(self.center)
        name = canvas.name_generator.get_unique_ellipse_name(self.center.name)        
        super().__init__(name=name, color=color, canvas=canvas)
        self._initialize()
    ...
    def get_state(self):
        ...
        ry = self.original_ry
        center = self.center.name
        state = {"name": self.name, "args": {"center": center, "radius_x": rx, "radius_y": ry}}
        return state


class Vector(Drawable):
    def __init__(self, origin_x, origin_y, tip_x, tip_y, canvas, name="", color=default_color):
    ...
    def get_state(self):
        ...
        state = {"name": self.name, "args": {"origin": origin, "tip": tip}}
        return state


class Function(Drawable):
    def __init__(self, function_string, left_bound, right_bound, canvas, name, step=default_point_size, color=default_color):
        name = canvas.name_generator.get_unique_function_name(name)
        ...
        self._create_points(left_bound, self.function(left_bound), right_bound, self.function(right_bound), canvas, color)
        super().__init__(name=name, color=color, canvas=canvas)
        self._initialize()

    def _create_points(self, start_x, start_y, end_x, end_y, canvas, color):
        start = canvas.get_point(start_x, start_y)
        if start:
            self.start_point = start
        else:
            self.start_point = Point(start_x, start_y, canvas, color=color)
            canvas.add_drawable(self.start_point)
        end = canvas.get_point(end_x, end_y)
        if end:
            self.end_point = end
        else:
            self.end_point = Point(end_x, end_y, canvas, color=color)
            canvas.add_drawable(self.end_point)
    ...
    def _initialize(self):
        self.start_point._initialize()
        self.end_point._initialize()
    ...
    def _generate_values(self):
        ...
        start_original = self.start_point.original_position
        start_scaled = self.start_point
        end_original = self.end_point.original_position
        end_scaled = self.end_point
    ...
    def _translate(self, offset):
        self.start_point._translate(offset)
        self.end_point._translate(offset)

    def _scale(self, factor):
        self.start_point._scale(factor)
        self.end_point._scale(factor)
    ...
    def get_state(self):
        ...
        state = {"name": self.name, "args": {"function_string": function_string, "start": start, "end": end}}
        return state