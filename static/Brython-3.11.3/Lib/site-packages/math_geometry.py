...
class Function(Drawable):
    ...
    def _generate_values(self):
        ...
        scale_factor = self.canvas.scale_factor
        f = self.function
        fstart = self.start.original_position
        start = self.start
        fend = self.end.original_position
        end = self.end
        steps = int((end.x + step - start.x) / step)
        scaled_step = (fend.x - fstart.x) / steps
        x_fvalues = [fstart.x + i * scaled_step for i in range(steps + 1)]
        x_values = [(x - fstart.x) * scale_factor + start.x for x in x_fvalues]
        y_fvalues = [f(x) for x in x_fvalues]
        y_values = [-((y - fstart.y) * scale_factor) + start.y for y in y_fvalues]
        values = [Position(x, y) for x, y in zip(x_values, y_values)]
        return values
