...
class Segment(Drawable):
    ...
    def _scale(self, factor):
        center_x, center_y = MathUtil.get_2D_midpoint(self.point1, self.point2)
        ...
        dx = self.point1.x - center_x
        dy = self.point1.y - center_y
        direction1 = (dx / length, dy / length)
        # Calculate the direction vector from the center to point2
        dx = self.point2.x - center_x
        dy = self.point2.y - center_y
        direction2 = (dx / length, dy / length)
        # Calculate the new positions of the points
        new_point1_x = center_x + direction1[0] * new_length
        new_point1_y = center_y + direction1[1] * new_length
        new_point2_x = center_x + direction2[0] * new_length
        new_point2_y = center_y + direction2[1] * new_length
...
class Triangle(Drawable):
    ...
    def _segments_form_triangle(self, s1, s2, s3):
        ...
        p2 = s2.point1
        p3 = s3.point1
        return p1 == s3.point2 and p2 == s1.point2 and p3 == s2.point2

    def _translate(self, offset):
        self.segment1.point1._translate(offset)
        self.segment2.point1._translate(offset)
        self.segment3.point1._translate(offset)
    ...
    def _scale(self, factor):
        ...
        centroid = Position(*MathUtil.get_triangle_centroid(p1, p2, p3)) # unpack the tuple into the Position constructor
