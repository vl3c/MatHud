...
class Segment(Drawable):
    ...
    def _calculate_line_algebraic_formula(self):
        p1 = self.point1
        p2 = self.point2
        line_formula = Utilities.calculate_line_formula(p1.original_position.x, p1.original_position.y, p2.original_position.x, p2.original_position.y)
        return line_formula
...
class Circle(Drawable):
    ...
    def _calculate_circle_algebraic_formula(self):
        ...
        r = self.radius
        circle_formula = Utilities.calculate_circle_formula(x, y, r)
...
class Ellipse(Drawable):
    ...
    def _calculate_ellipse_algebraic_formula(self):
        ...
        rx = self.radius_x
        ry = self.radius_y
        ellipse_formula = Utilities.calculate_ellipse_formula(x, y, rx, ry)
...
class Function(Drawable):
    ...
    def draw(self):
        ...
        # Check if xy has at least two elements
        if len(xy) < 2:
            return
        # Draw function name
        offset_x = (1 + len(self.name)) * point_label_font_size / 2
        label_x = xy[1].x - offset_x
        label_y = xy[1].y
        self.create_svg_element('text', x=str(label_x), y=str(label_y), fill=self.color, text_content=self.name, text_font_size=point_label_font_size)
        # Draw function
