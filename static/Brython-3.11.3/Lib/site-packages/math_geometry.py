...
class Function(Drawable):
    ...
    def _should_regenerate_points(self):
        ...
        # Always regenerate on zoom
        if self._last_scale != current_scale:
            self._last_scale = current_scale
            self._last_bounds = current_bounds
            print("Zoom detected: Regenerating points.")  # Debug statement
            return True

        # Regular cache invalidation for bounds changes or first draw
        if (self._cached_points is None or 
            not self._cache_valid or 
            self._last_bounds != current_bounds):
            self._last_scale = current_scale
            self._last_bounds = current_bounds
            print("Cache Invalidation: Regenerating points.")  # Debug statement
            return True

        print("No Regeneration Needed.")  # Debug statement
        return False
    ...
    def _generate_values(self):
        ...
        # Increase max_points with zoom level to maintain smoothness
        base_max_points = 200
        max_points = min(base_max_points * math.ceil(scale_factor), 1000)  # Cap at 1000 points
        
        def evaluate_point(x):
            try:
                y = f(x)
                if bottom_bound <= y <= top_bound:
                    return self._original_to_scaled(x, y)
                return None
            except (ZeroDivisionError, ValueError):
                return None

        # Calculate step size based on visible range and current zoom level
        range_width = right_bound - left_bound
        step = range_width / max_points
        
        x = left_bound
        last_point = None
        while x <= right_bound and len(points) < max_points:
            point = evaluate_point(x)
            if point:
                # Add point if it's visibly different at current resolution
                pixel_diff = 1  # minimum pixel difference to add point
