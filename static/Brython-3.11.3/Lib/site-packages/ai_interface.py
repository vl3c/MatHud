...
class AIInterface:
    ...
    def run_tests(self):
        ...
        def test_undoable_functions(self):
        ...
        def test_create_ai_message():
            test_cases = [
                ...
                ("Test response.", [42, "result"], "Test response., 42, result")
            ]
            for ai_response, call_results, expected_output in test_cases:
                output = self._create_ai_message(ai_response, call_results)
        ...
        def test_build_prompt():
            ...
            assert type(prompt) == str and "canvas_state" in prompt, "Prompt should be a string containing 'canvas_state'"

        def test_set_function_call_result():
            # Validate and update the function_call_results list with valid results
            self.function_call_results.clear()
            valid_results = [1, "string", 3.14, True]
            ProcessFunctionCalls.set_function_call_result(valid_results, self.function_call_results)
            assert self.function_call_results == valid_results, "function_call_results should be updated with valid results"
            # Validate and update the function_call_results list with invalid results
            self.function_call_results.clear()
            invalid_results = [1, [], 3.14, True]  # Lists are not allowed
            ProcessFunctionCalls.set_function_call_result(invalid_results, self.function_call_results)
            assert not self.function_call_results, "function_call_results should be empty with invalid results"

        # Run the class unit tests
        test_graphics_drawing()
        test_undoable_functions(self)
        test_create_ai_message()
        test_build_prompt()
        test_set_function_call_result()
        tests.run_tests()

    def _create_ai_message(self, ai_response, call_results):
        # This function should return the text part of the AI's reply
        ai_response_text = ai_response if ai_response else "..."
        # Check if call results are valid and should be appended to the AI response
        if call_results and ProcessFunctionCalls.validate_results(call_results):
            call_results_text = ', '.join(map(str, call_results))
            # Append call results to ai_response if it exists; otherwise, use call results as the response
            if ai_response_text != "...":
                ai_response_text += ', ' + call_results_text
            else:
                ai_response_text = call_results_text
        elif not ai_response_text:
            # If there is no ai_response and call_results is not valid or empty, return the placeholder
            ai_response_text = "..."
    ...
    def _parse_ai_response(self, ai_message, ai_function_calls):
        try:
            ...
            ProcessFunctionCalls.set_function_call_result(call_results, self.function_call_results)
            call_results = self.function_call_results
            # Get the text part of the AI's reply
            ai_response_text = self._create_ai_message(ai_message, call_results).replace('\n', '<br>')               
    ...
    def _build_prompt(self, canvas_state, function_call_results, user_message):
