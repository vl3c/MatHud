...
from config import successful_call_message
import json
...
class AIInterface:
    def __init__(self, canvas):
        ...
        self.undoable_functions = ("clear_canvas", "reset_canvas", "create_point", "delete_point", "create_segment", "delete_segment", "create_vector", "delete_vector", "create_triangle", "delete_triangle", "create_rectangle", "delete_rectangle", "create_circle", "delete_circle", "create_ellipse", "delete_ellipse", "draw_function", "delete_function", "translate_object")
    ...
    def _create_ai_response_text(self, ai_response, call_results):
        ...
        if call_results and ProcessFunctionCalls.validate_results(call_results):
            call_results_text = '\n'.join([f"{key}: {value}" for key, value in call_results.items() if not (isinstance(value, str) and \
                                                                                                            (value in [successful_call_message, "",] or value.startswith("Error:")))])
    ...
    def _create_ai_intermediate_request_text(self, ai_response, call_results):
        if not isinstance(call_results, dict):
            raise ValueError("Invalid input for call_results. Expected a dictionary.")
        message = "I requested some intermediate results:"
        ai_response_text = f"{ai_response}n\{message}" if ai_response else message
        if call_results and ProcessFunctionCalls.validate_results(call_results):
            call_results_text = '\n'.join([f"{key}: {value}" for key, value in call_results.items() if not (isinstance(value, str) and \
                                                                                                            (value in [successful_call_message, "",] or value.startswith("Error:")))])
            if call_results_text:  # If there's meaningful call_results_text
    ...
    def _parse_ai_response(self, ai_message, ai_function_calls):
        try:
            ...
            call_results, ai_requested_intermediate_results = ProcessFunctionCalls.get_results(ai_function_calls, self.available_functions, self.undoable_functions, self.canvas)
            # Update the function call results if they are valid
            if ProcessFunctionCalls.validate_results(call_results):
                self.function_call_results.update(call_results)  
            
            if ai_requested_intermediate_results and call_results:
                ai_response_text = self._create_ai_intermediate_request_text(ai_message, call_results).replace('\n', '<br>')
            else:
                ai_response_text = self._create_ai_response_text(ai_message, call_results).replace('\n', '<br>')
            ...
            if ai_requested_intermediate_results and call_results:
                # Send the AI the prompt for the intermediate results
    ...
    def _send_prompt_to_ai(self, user_message=""):
        ...
        # Clean up the function call results
        self.function_call_results = {k: v for k, v in self.function_call_results.items() if not (isinstance(v, str) and \
                                                                                                  (v in [successful_call_message, "",] or v.startswith("Error:")))}
