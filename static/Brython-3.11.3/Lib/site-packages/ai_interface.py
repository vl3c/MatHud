        ...
        try:
            ...
            if ProcessFunctionCalls.validate_results(call_results):
                for key, value in call_results.items():
                    if not (isinstance(value, str) and (value in [successful_call_message, "",] or value.startswith("Error:"))):
            ...
            if self.multi_step_mode_enabled and call_results:
                ai_response_text = self._create_ai_intermediate_request_text(ai_message, call_results)
            else:
            ...
            if self.multi_step_mode_enabled and not call_results:
                self._send_prompt_to_ai("Please proceed with the next steps in solving this problem.")
            elif self.multi_step_mode_enabled and call_results:
