...
class AIInterface:
    ...
    def _create_ai_response_text(self, ai_response, call_results):
        ...
        # Check if any result has the _no_response suffix
        if any(isinstance(value, str) and value.endswith("_no_response") for value in call_results.values()):
            return ""
    ...
    def save_workspace(self, name=None):
        ...
        def on_complete(req):
            try:
                ...
                document["chat-history"].scrollTop = document["chat-history"].scrollHeight
                print(error_message)
            return successful_call_message + "_no_response"
    ...
    def list_workspaces(self):
        ...
        def on_complete(req):
            try:
                ...
                if workspaces:
                    workspace_list = ', '.join(workspaces)
                    message = f'<strong>Available workspaces:</strong> {workspace_list}'
                else:
                    message = '<strong>Available workspaces:</strong> None'
