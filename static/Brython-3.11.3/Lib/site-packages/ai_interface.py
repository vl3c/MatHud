...
class AIInterface:
    ...
    def _create_ai_intermediate_request_text(self, call_results):
        if not isinstance(call_results, dict):
            raise ValueError("Invalid input for call_results. Expected a dictionary.")
            ...
            for key, value in call_results.items():
                ...
                if not ProcessFunctionCalls.is_successful_result(value):
                    ...
                    if isinstance(value, (int, float)):
                        ...
                        successful_results.append(f"{key} = {formatted_value}")
                    else:
    ...
    def _parse_ai_response(self, ai_message, ai_function_calls, finish_reason):
        try:
            # Log debug information about the AI response
            self._debug_log_ai_response(ai_message, ai_function_calls, finish_reason)
            ...
            try:
                ...
                # Check for rotation errors in the results
                for key, value in call_results.items():
                    if isinstance(value, str) and "Would you like to rotate" in value:
                        # Send the error message to the AI
                        self._send_prompt_to_ai(f"System: {value}. Please ask the user if they want to rotate the parent shape instead.")
                        return
                        
            except ValueError as e:
                error_msg = str(e)
                print(f"Error processing function calls: {error_msg}")  # DEBUG
            ...
            if self.multi_step_mode_enabled and call_results:
                intermediate_results = self._create_ai_intermediate_request_text(call_results)
            ...
            if ai_response_text:
                self._print_ai_message_in_chat(ai_response_text)
            ...
            if self.multi_step_mode_enabled:
                if not call_results or ("multi_step_mode" in call_results and len(call_results) == 1):
                    self._send_prompt_to_ai("System: Please proceed with the next steps in solving this problem. DO NOT call enable_multi_step_mode by itself again.")
                else:
                    self._send_prompt_to_ai("System: I've saved the results of your calculations in the canvas state. You can now use these results in your next actions. Please proceed with the next steps in solving this problem.")
    ...
    def _process_ai_response(self, ai_message, tool_calls, finish_reason):     
        ...
        if finish_reason == "stop":
            ...
            call_results, _ = ProcessFunctionCalls.get_results(tool_calls, self.available_functions, self.undoable_functions, self.canvas)
            self._store_results_in_canvas_state(call_results)
