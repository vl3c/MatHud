    ...
    def get_results(calls, available_functions, undoable_functions, canvas):
        ...
        for call in calls:
            ...
            try:
                ...
                # Skip appending if the result is None
                if result is None:
                    continue
                # Format the result string differently for 'evaluate_expression'
                if function_name == 'evaluate_expression':
                    expression_and_result = f"{args['expression']} = {result}"
                else:
                    expression_and_result = f"{function_name}({', '.join(f'{k}={v}' for k, v in args.items())}) = {result}"
